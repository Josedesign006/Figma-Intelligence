...
export type BuildStep =
  | { type: "moveCursor";   agentId: string; x: number; y: number }
  | { type: "showChat";     agentId: string; text: string }
  | { type: "hideChat";     agentId: string }
...
function buildCursorInitScript(agents: AgentDef[]): string {
  return `
(async () => {
...
function buildStepScript(step: BuildStep): string {
  switch (step.type) {
    case "moveCursor":
      return `
(async () => {
