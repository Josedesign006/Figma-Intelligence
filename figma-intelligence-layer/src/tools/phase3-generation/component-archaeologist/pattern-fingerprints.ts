...
export function inferSignal(node: FigmaNode): LayerSignal | null {
  const name = node.name.toLowerCase();
  if (/\bicon\b|\bsvg\b|\bvector\b/.test(name) || node.type === "VECTOR") return "icon";
  if (/\bavatar\b|\bprofile\s*pic\b|\bthumb(nail)?\b/.test(name)) return "avatar";
  if (/\bimage\b|\bphoto\b|\billustration\b|\bhero\b/.test(name) || node.type === "IMAGE" as string) return "image";
  if (/\binput\b|\bfield\b|\btextarea\b|\bsearch\b/.test(name)) return "input";
  if (/\bchevron\b|\barrow\b|\bcaret\b/.test(name)) return "chevron";
  if (/\btitle\b|\bheading\b|\bh[1-6]\b/.test(name)) return "title";
  if (/\bsubtitle\b|\bsub\s*heading\b|\bsecondary\s*text\b/.test(name)) return "subtitle";
  if (/\bbody\b|\bdescription\b|\bparagraph\b|\bcontent\b/.test(name)) return "body";
  if (/\blabel\b|\bcaption\b/.test(name)) return "label";
  if (/\bbtn\b|\bbutton\b|\bcta\b/.test(name)) return "cta";
...
export function buildLayerSummary(root: FigmaNode): LayerSummary {
  let totalLayers = 0;
  let maxDepth = 0;
  let textCount = 0;
  ...
  let frameCount = 0;
  let vectorCount = 0;
  const signals = new Set<LayerSignal>();

  function walk(node: FigmaNode, depth: number): void {
    totalLayers++;
    maxDepth = Math.max(maxDepth, depth);
    if (node.type === "TEXT") textCount++;
    if (node.type === "RECTANGLE") rectangleCount++;
    if (node.type === "FRAME" || node.type === "GROUP") frameCount++;
    if (node.type === "VECTOR") vectorCount++;
...
export const PATTERN_RULES: PatternRule[] = [
  {
    ...
    test: (s, sigs) =>
      s.layoutMode === "HORIZONTAL" && sigs.includes("icon") && sigs.includes("text"),
  },
  {
    ...
    test: (s, sigs) =>
      s.hasImage && sigs.includes("title") && (sigs.includes("body") || s.textCount >= 3),
  },
  {
    pattern: "Label + Input",
    ...
    test: (s, sigs) =>
      sigs.includes("label") && sigs.includes("input"),
  },
  {
    ...
    test: (s, sigs) =>
      sigs.includes("avatar") && s.textCount >= 2,
  },
  {
    ...
    test: (s, sigs) =>
      sigs.includes("icon") && sigs.includes("title") && sigs.includes("chevron"),
  },
  {
    ...
    test: (s) =>
      s.childCount >= 3 && s.layoutMode === "HORIZONTAL",
  },
  {
    ...
    test: (s, sigs) =>
      sigs.includes("cta") && s.totalLayers <= 5,
  },
  {
    pattern: "Text stack",
    ...
    test: (s) =>
      s.textCount >= 2 && s.textCount === s.childCount && s.layoutMode === "VERTICAL",
  },
];
...
export function matchPatterns(
  ...
  root: FigmaNode
): Array<{ pattern: string; candidates: string[]; confidence: number; rationale: string }> {
