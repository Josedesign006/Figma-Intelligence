...
import {
  ...
  NavigationType,
  buildWireScript,
  buildClearReactionsScript,
} from "../../../shared/prototype-script-builder.js";
...
export async function prototypeScanHandler(
  args: PrototypeScanArgs
): Promise<PrototypeScanResult> {
  const {
    frameIds,
    journeyDescription,
    maxDepth = 5,
  ...
  const script = buildScanScript(
    frameIds && frameIds.length > 0 ? frameIds : null,
    Math.min(maxDepth, 8),
    20,
    5000
  );
  ...
  const totalInteractiveElements = data.frames.reduce(
    (sum, f) => sum + f.interactiveElements.length,
    0
  );
...
export async function prototypeWireHandler(
  args: PrototypeWireArgs
): Promise<PrototypeWireResult> {
  const {
    connections,
    journeyDescription,
    frameIds,
  ...
  if (!connections || connections.length === 0) {
    ...
    throw new Error(
      "Either 'connections' array or 'journeyDescription' must be provided."
    );
  }
...
`.trim();
  ...
  const nodeNames: Record<string, string> =
    nameResult.success && nameResult.result
      ? (nameResult.result as Record<string, string>)
      : {};

  // Build WireSpec array
  const defAnim = defaultAnimation || {};
  const wireSpecs: WireSpec[] = connections.map((c) => ({
  ...
  if (clearExisting) {
    ...
    const clearScript = buildClearReactionsScript(uniqueFromIds);
    await bridge.execute(clearScript);
  }

  // Execute wiring
  const wireScript = buildWireScript(wireSpecs);
  ...
  const scriptResults = wireResult.result as Array<{
    fromNodeId: string;
    success: boolean;
    error?: string;
  }>;
  ...
  const wiredConnections: WireResultEntry[] = scriptResults.map((r, i) => ({
