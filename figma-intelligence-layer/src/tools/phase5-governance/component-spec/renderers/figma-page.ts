...
import type { ComponentSpec, SpecSectionContent, SpecSectionOutput } from "../types.js";
...
function formatTable(headers: string[], rows: string[][]): string {
  ...
  const widths = headers.map((_, ci) =>
    Math.min(Math.max(...all.map(r => (r[ci] || "").length), 3), 45)
  );
  const hdr = headers.map((h, i) => h.padEnd(widths[i])).join("    ");
  const sep = widths.map(w => "\u2500".repeat(w)).join("\u2500\u2500\u2500\u2500");
  const body = rows.slice(0, 50).map(row =>
    row.map((cell, i) => (cell || "\u2014").padEnd(widths[i] || 4)).join("    ")
  );
  return [hdr, sep, ...body].join("\n");
}

function contentToString(content: SpecSectionContent): string {
  switch (content.kind) {
    case "key-value":
    ...
    case "structured-data":
      return formatTable(
        content.columns,
        content.rows.map(r => content.columns.map(c => r[c] ?? "\u2014"))
      );
...
async function init(bridge: Bridge, pageName: string): Promise<{ pageId: string; mId: string }> {
...
async function renderHeader(bridge: Bridge, mId: string, name: string, subtitle: string): Promise<void> {
  const r = await bridge.execute(`
    (async () => {
      ${FL}
  ...
  `);
  if (!r.success) console.error("Header failed:", r.error);
}
...
async function renderSection(
  ...
  `);
  if (!r.success) console.error(`Section "${title}" failed:`, r.error);
}

// ── Main entry ─────────────────────────────────────────────────────────────

export async function renderFigmaPage(spec: ComponentSpec, pageName?: string): Promise<string> {
