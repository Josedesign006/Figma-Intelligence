...
import type { ComponentSpec, SpecSectionContent } from "../types.js";
import { renderAnatomyDiagram } from "./anatomy-diagram.js";
...
async function renderDocHeader(
  bridge: Bridge, mId: string, name: string,
  description: string, sectionTitles: string[],
): Promise<void> {
  ...
  const r1 = await bridge.execute(`
    (async () => {
      ...
      // H1 title
      ${textFillJS("t", "m", "b", 40, 52, esc(name), C.textMain,
        `t.letterSpacing={unit:"PIXELS",value:-1};`)}
  ...
  `);
  if (!r1.success) console.error("Header text failed:", r1.error);
  ...
  `);
  if (!r2.success) console.error("Header badges failed:", r2.error);
}

// ── renderSectionTitle ────────────────────────────────────────────────────

async function renderSectionTitle(
  bridge: Bridge, mId: string, title: string, index: number,
): Promise<void> {
  const r = await bridge.execute(`
    (async () => {
      ${FL}
  ...
  `);
  if (!r.success) console.error(`Section title "${title}" failed:`, r.error);
}

// ── Sub-renderers ─────────────────────────────────────────────────────────

async function renderVisualParagraph(
  bridge: Bridge, mId: string, text: string,
): Promise<void> {
  const r = await bridge.execute(`
    (async () => {
      ${FL}
  ...
  `);
  if (!r.success) console.error("Paragraph failed:", r.error);
}

async function renderVisualList(
  bridge: Bridge, mId: string, items: string[],
): Promise<void> {
  ...
  `);
  if (!r.success) console.error("List failed:", r.error);
}

async function renderVisualKeyValue(
  bridge: Bridge, mId: string, entries: Array<{ label: string; value: string }>,
): Promise<void> {
  ...
  `);
  if (!r.success) console.error("Key-value failed:", r.error);
}

async function renderVisualTable(
  bridge: Bridge, mId: string, headers: string[], rows: string[][],
): Promise<void> {
  ...
  `);
  if (!r.success) console.error("Table failed:", r.error);
}

async function renderVisualDoDont(
  bridge: Bridge, mId: string, dos: string[], donts: string[],
): Promise<void> {
  ...
  `);
  if (!r.success) console.error("Do/Dont failed:", r.error);
}

// ── Content kind router ───────────────────────────────────────────────────

async function renderSectionContent(
  bridge: Bridge, mId: string, content: SpecSectionContent,
): Promise<void> {
  switch (content.kind) {
    case "paragraph":
...
export async function renderVisualDoc(spec: ComponentSpec, pageName?: string): Promise<string> {
  ...
  if (spec.extraction.anatomy.elements.length > 0) {
    try {
      ...
      await renderAnatomyDiagram(bridge, pageId, spec.nodeId, spec.extraction.anatomy, mH + 60);
    } catch (err) {
