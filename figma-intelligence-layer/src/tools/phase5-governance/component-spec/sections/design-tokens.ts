/**
 * Design Token Bindings section — extracted color tokens + knowledge token map
 */
import type { ExtractionResult, SpecSectionOutput, SpecSectionContent } from "../types.js";

export function buildDesignTokensSection(data: ExtractionResult): SpecSectionOutput | null {
  const { colorTokens, knowledge } = data;
  if (!knowledge) return null;
  if (knowledge.designTokenBindings.length === 0 && colorTokens.length === 0) return null;

  const blocks: SpecSectionContent[] = [];

  // Start with extracted color tokens mapped to structured format
  const extractedRows: Record<string, string>[] = colorTokens
  ...
  const knowledgeRows = knowledge.designTokenBindings.filter(
    (kb) => !coveredProperties.has(kb.property.toLowerCase())
  );
