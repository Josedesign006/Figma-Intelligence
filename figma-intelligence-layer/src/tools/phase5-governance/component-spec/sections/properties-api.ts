/**
 * Properties / API section — all component properties in a unified table
 */
import type { ExtractionResult, SpecSectionOutput, SpecSectionContent } from "../types.js";

export function buildPropertiesSection(data: ExtractionResult): SpecSectionOutput {
  const { properties, snapshot } = data;
  const blocks: SpecSectionContent[] = [];

  const hasProperties =
    properties.variantAxes.length > 0 ||
  ...
  if (hasProperties) {
    ...
    if (hasText) suggestions.push("Add text override properties for editable labels");
    if (hasInstances) suggestions.push("Add instance swap properties for icon/sub-component slots");
