/**
 * Related Components section — cross-references based on component type
 */
import type { ExtractionResult, SpecSectionOutput, SpecSectionContent } from "../types.js";

export function buildRelatedSection(data: ExtractionResult): SpecSectionOutput {
  const blocks: SpecSectionContent[] = [];

  // Map component types to related alternatives
  const relationships: Record<string, Array<{ name: string; relationship: string }>> = {
    button: [
