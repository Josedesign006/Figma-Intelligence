...
import { COMPONENT_BLUEPRINTS, getBlueprint, type ComponentBlueprint, type BlueprintNode } from "../../../shared/component-templates.js";
import { SEMANTIC_TOKEN_CATALOG } from "../../../shared/semantic-token-catalog.js";
import { TYPOGRAPHY_PRESETS, getPreset } from "../../../shared/typography-presets.js";
import { STATE_OVERRIDES, SIZE_OVERRIDES, TYPE_OVERRIDES, type OverrideMap } from "../../phase2-accuracy/variant-expander/token-override-maps.js";
import { generateValidatorScript, generateDocumentRepairScript } from "../../../shared/auto-layout-validator.js";
...
export interface ContentOverrides {
  ...
  qaChecklist?: string[] | Array<{ area: string; verify: string; expected: string }>;
  hierarchy?: string;
  structureAndSpacing?: string;
  relatedComponents?: Array<{ name: string; relationship: string; whenToPrefer: string }>;
}
...
function resolveBlueprint(name: string): ComponentBlueprint | undefined {
  // Exact match first
  const exact = getBlueprint(name);
  ...
  const fuzzy = getBlueprint(stripped);
  if (fuzzy) return fuzzy;
  // Try each blueprint name as a substring match
  const lower = name.toLowerCase();
  return COMPONENT_BLUEPRINTS.find((b) => lower.includes(b.name.toLowerCase()));
}

function collectBlueprintTokens(node: BlueprintNode): string[] {
  const tokens: string[] = [];
  if (node.fillSemantic) tokens.push(node.fillSemantic);
  if (node.strokeSemantic) tokens.push(node.strokeSemantic);
  if (node.textFillSemantic) tokens.push(node.textFillSemantic);
  if (node.children) {
...
function collectBlueprintChildren(node: BlueprintNode): BlueprintNode[] {
...
// Deep per-component knowledge that cannot be derived from blueprint structure alone.
// Covers interaction patterns, semantic HTML, accessibility, and content rules.
...
const COMPONENT_KNOWLEDGE: Record<string, ComponentKnowledge> = {
  button: {
...
function inferWhenToUse(name: string): string[] {
  ...
  if (knowledge) return knowledge.whenToUse;
  return [
...
function inferWhenNotToUse(name: string): string[] {
  ...
  const bp = resolveBlueprint(name);
  const knowledge = COMPONENT_KNOWLEDGE[n] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.whenNotToUse;
  return [
...
function buildAnatomy(snapshot: NodeSnapshot): DesignSystemSpec["anatomy"] {
  ...
  if (bp && bp.root.children) {
    return bp.root.children.map((child, i) => {
      ...
      if (child.fillSemantic) description += ` Bound to token ${child.fillSemantic}.`;
      if (child.textPreset) {
        const preset = getPreset(child.textPreset);
        if (preset) description += ` Typography: ${preset.name} (${preset.size}px ${preset.weight}).`;
      }
      if (hasA11y && /icon/i.test(child.name)) description += " If icon-only (no adjacent label), requires aria-label for screen reader access.";
      return {
        index: i + 1,
        name: child.name,
...
function inferAnatomyDescription(name: string): string {
  const n = name.toLowerCase();
  if (/^icon$|^leading.?icon$/i.test(n)) return "Leading visual icon reinforcing the label or indicating the action type. Typically 16×16px.";
  if (/trailing.?icon/i.test(n)) return "Trailing icon providing supplementary visual context or indicating expandable state (e.g., chevron).";
  if (/^icon/i.test(n)) return "Visual icon element supporting the label or indicating an action.";
  if (/^label$|^title$/i.test(n)) return "Primary text label communicating the component's purpose to the user. This is the accessible name source.";
  if (/heading/i.test(n)) return "Section heading providing hierarchical structure and scannability.";
  if (/body|description|subtitle/i.test(n)) return "Supporting text providing additional context, limited to 2–3 lines for scannability.";
  if (/value|placeholder/i.test(n)) return "Text content area displaying the current value or placeholder guidance.";
  if (/image|thumbnail|media|photo/i.test(n)) return "Visual media slot for images, illustrations, or video thumbnails. Requires alt text if meaningful.";
  if (/container|wrapper|frame|content/i.test(n)) return "Layout container organizing child elements with auto-layout and consistent padding.";
  if (/divider|separator/i.test(n)) return "Visual separator between content sections using border/subtle token.";
  if (/badge|indicator|status.?dot/i.test(n)) return "Status or count indicator element communicating state metadata.";
  if (/field/i.test(n)) return "Input field container with border, background, and focus state styling.";
  if (/button|btn|action|cta/i.test(n)) return "Interactive action element triggering a specific operation.";
  if (/close|dismiss/i.test(n)) return "Dismiss control allowing the user to close or remove the component. Requires aria-label='Close'.";
  if (/chevron|arrow|caret/i.test(n)) return "Directional indicator showing expandable/collapsible state or navigation direction.";
  if (/header/i.test(n)) return "Header region containing title and optional controls (close button, actions).";
  if (/footer/i.test(n)) return "Footer region containing action buttons, typically right-aligned with primary action last.";
  if (/spacer/i.test(n)) return "Layout spacer maintaining consistent spacing between elements.";
  if (/thumb/i.test(n)) return "Draggable thumb element indicating and controlling the current state position.";
  if (/track/i.test(n)) return "Track element providing the rail along which the thumb moves.";
  if (/box|circle/i.test(n)) return "Visual indicator element showing the current selection state (checked/unchecked/selected).";
  if (/nav/i.test(n)) return "Navigation items container holding the primary navigation links.";
  if (/logo/i.test(n)) return "Brand identity element linking to the application's home page.";
  if (/avatar/i.test(n)) return "User identity display showing photo, initials, or fallback icon.";
  if (/initials/i.test(n)) return "Text fallback displaying 1–2 uppercase characters when no profile photo is available.";
  if (/remove/i.test(n)) return "Remove action control for dismissible tags/chips. Requires aria-label='Remove [item name]'.";
  if (/row/i.test(n)) return "Data row container holding cell values for a single data entry.";
  if (/col|cell/i.test(n)) return "Data cell containing a single value within a table column.";
  if (/message/i.test(n)) return "Message text area delivering the notification content.";
...
function buildPropsTable(snapshot: NodeSnapshot): PropsEntry[] {
  ...
  if (bp) {
    for (const vp of bp.variantProperties) {
      ...
      if (propName === "state") {
        ...
        description = `Controls the component's dimensions, padding, and font size. Each size maps to the size scale: ${vp.values.map(v => { const so = SIZE_OVERRIDES[v.toLowerCase()]; return so ? `${v} (${so.find(o => o.property === "height")?.rawValue || "?"}px)` : v; }).join(", ")}.`;
      } else if (propName === "type") {
        description = `Controls the visual emphasis and semantic meaning. ${vp.values.map(v => { const to = TYPE_OVERRIDES[v.toLowerCase()]; return to ? `${v}: ${to.map(o => o.property).join(", ")}` : v; }).join(". ")}.`;
      } else {
  ...
  for (const [property, values] of Object.entries(snapshot.variantGroupProperties)) {
    if (props.some((p) => p.name === property)) continue;
    props.push({
      name: property,
      type: "variant",
      values,
...
function buildPurpose(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.purpose;
  if (bp) return bp.description;
  ...
  for (const [property, values] of Object.entries(snapshot.variantGroupProperties)) {
    if (/size|scale|density/i.test(property)) {
      for (const val of values) {
        ...
        const override = SIZE_OVERRIDES[v];
        ...
        if (/xs|extra.?small/i.test(v)) { useCase = `Dense data tables, compact toolbars, inline secondary actions. Height: ${height || 24}px, padding: ${paddingY || 2}/${paddingX || 4}px, font: ${fontSize || 12}px.`; context = "Desktop-only, information-dense layouts. Do not use for primary CTAs or on mobile."; minTouchTarget = "24×24px visual, but surrounding spacing must reach 44px effective target"; }
        else if (/sm|small/i.test(v)) { useCase = `Secondary actions, toolbar controls, table row actions. Height: ${height || 32}px, padding: ${paddingY || 4}/${paddingX || 8}px, font: ${fontSize || 14}px.`; context = "Dense desktop layouts, data tables, toolbars, inline actions within larger components."; minTouchTarget = "32×32px visual — pair with 8px+ spacing to approach 44px effective target on mobile"; }
        else if (/md|medium|default/i.test(v)) { useCase = `Standard UI interactions, form controls, primary buttons. Height: ${height || 40}px, padding: ${paddingY || 8}/${paddingX || 16}px, font: ${fontSize || 16}px.`; context = "Most screens, forms, content areas, and modal actions. The default choice for most use cases."; minTouchTarget = "40×40px visual — close to 44px target; ensure adequate touch padding"; }
        else if (/lg|large/i.test(v)) { useCase = `Primary mobile actions, hero CTAs, high-priority form submissions. Height: ${height || 48}px, padding: ${paddingY || 16}/${paddingX || 24}px, font: ${fontSize || 18}px.`; context = "Mobile primary actions, landing page CTAs, prominent form submit buttons."; minTouchTarget = "48×48px — exceeds WCAG minimum"; }
        else if (/xl|extra.?large/i.test(v)) { useCase = `Full-width mobile actions, hero sections, onboarding CTAs. Height: ${height || 56}px, padding: ${paddingY || 24}/${paddingX || 32}px, font: ${fontSize || 20}px.`; context = "Mobile-first layouts, marketing pages, high-priority first-action surfaces."; minTouchTarget = "56×56px — generous target for mobile"; }
...
function buildBehaviour(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.behaviour;
  return "";
}
...
function buildInteractionRules(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.interactionRules;
  return "";
}
...
function buildContentGuidance(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.contentGuidance;
  if (bp) {
...
function buildResponsive(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.responsive;
  if (bp) {
...
function buildImplementationNotes(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
  if (knowledge) return knowledge.implementationNotes;
  if (bp) {
...
function buildQaChecklist(snapshot: NodeSnapshot): string[] {
  ...
  for (const state of states) {
    const overrides = STATE_OVERRIDES[state.toLowerCase()];
...
function buildHierarchyAndEmphasis(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  if (!bp) return "";
  ...
  for (const type of sortedTypes) {
    const overrides = TYPE_OVERRIDES[type.toLowerCase()];
...
function buildStructureAndSpacing(snapshot: NodeSnapshot): string {
  const bp = resolveBlueprint(snapshot.name);
  if (!bp) return "";
  ...
  const nodeTokens: Array<{ element: string; property: string; token: string }> = [];
  ...
  if (root.fillSemantic) nodeTokens.push({ element: "Container", property: "fills", token: root.fillSemantic });
  if (root.strokeSemantic) nodeTokens.push({ element: "Container", property: "strokes", token: root.strokeSemantic });
  if (root.children) {
    for (const child of root.children) {
      if (child.fillSemantic) nodeTokens.push({ element: child.name, property: "fills", token: child.fillSemantic });
      if (child.strokeSemantic) nodeTokens.push({ element: child.name, property: "strokes", token: child.strokeSemantic });
      if (child.textFillSemantic) nodeTokens.push({ element: child.name, property: "text fills", token: child.textFillSemantic });
    }
  }
...
function buildStatesDetailed(snapshot: NodeSnapshot): DetailedState[] {
  const states = inferStatesFromSnapshot(snapshot);
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];

  return states.map((stateName) => {
    const overrides = STATE_OVERRIDES[stateName.toLowerCase()];
...
function buildAccessibilityDeep(snapshot: NodeSnapshot): AccessibilityDeep {
  const bp = resolveBlueprint(snapshot.name);
  const knowledge = COMPONENT_KNOWLEDGE[snapshot.name.toLowerCase()] || COMPONENT_KNOWLEDGE[bp?.name.toLowerCase() || ""];
...
function buildRelatedComponents(snapshot: NodeSnapshot): RelatedComponent[] {
  const bp = resolveBlueprint(snapshot.name);
  if (!bp) return [];
  ...
  const sameCategory = COMPONENT_BLUEPRINTS.filter(
    (b) => b.category === bp.category && b.name !== bp.name
  );
  ...
  if (args.contentOverrides) {
    ...
    if (co.qaChecklist) {
      if (Array.isArray(co.qaChecklist) && co.qaChecklist.length > 0 && typeof co.qaChecklist[0] === "object") {
        spec.qaChecklist = (co.qaChecklist as Array<{ area: string; verify: string; expected: string }>)
          .map((q) => `[${q.area}] ${q.verify} → ${q.expected}`);
      } else {
        spec.qaChecklist = co.qaChecklist as string[];
      }
    }
    if (co.hierarchy) spec.hierarchy = co.hierarchy;
    if (co.structureAndSpacing) spec.structureAndSpacing = co.structureAndSpacing;
    if (co.relatedComponents) spec.relatedComponents = co.relatedComponents;
    if (co.variants) spec.variantsDetailed = co.variants;
    if (co.supportedCompositions) spec.supportedCompositions = co.supportedCompositions;
    if (co.accessibility) {
