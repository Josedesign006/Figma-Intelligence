...
// ─── Annotation Type → Stamp Icon & Label ─────────────────────────────────

export interface AnnotationTypeDef {
  key: string;
  label: string;
  icon: string;        // Unicode character for the icon in the stamp
  showNumber: boolean;
  detailsTitle: string;
...
export const STAMP_SPECS = {
  ...
  lassoStrokeWidth: 2,
  lassoDashPattern: [6, 4] as number[],
  lassoCornerRadius: 4,
  connectorStrokeWidth: 1.5,
  connectorDashPattern: [4, 4] as number[],
...
export const DETAILS_CARD_SPECS = {
  width: 260,
  headerHeight: 32,
  rowHeight: 28,
  padding: 12,
...
// ─── Element Classification for Annotation Mapping ───────────────────────
...
export function roleToAnnotationType(role: string): AnnotationTypeKey | null {
  switch (role) {
    case "button":
      return "button";
    case "link":
...
export function inferInputType(name: string): string {
  const n = name.toLowerCase();
  if (/email/i.test(n)) return "email";
  if (/password/i.test(n)) return "password";
  if (/phone|tel/i.test(n)) return "tel";
  if (/number|qty|quantity/i.test(n)) return "number";
  if (/date/i.test(n)) return "date";
  if (/time/i.test(n)) return "time";
  if (/search/i.test(n)) return "search";
  if (/url|website/i.test(n)) return "url";
  if (/checkbox|check/i.test(n)) return "checkbox";
  if (/radio/i.test(n)) return "radio";
  if (/select|dropdown|combo/i.test(n)) return "select";
  if (/textarea|multiline/i.test(n)) return "textarea";
  if (/fieldset|field-set|form.*group/i.test(n)) return "fieldset";
  return "text";
}
