...
import type { VPATReport, VPATRow } from "./vpat-report.js";
import type { WCAGPrinciple } from "./wcag-criteria.js";
...
function statusColors(status: string): { text: string; bg: string } {
  switch (status) {
...
export async function renderVPATPage(
  bridge: Bridge,
  report: VPATReport,
  ...
  `);
  if (!initResult.success) throw new Error(`VPAT init: ${initResult.error}`);
  ...
  const principleOrder: WCAGPrinciple[] = ["Perceivable", "Operable", "Understandable", "Robust"];
...
async function renderPrincipleTable(
  bridge: Bridge,
  mId: string,
  principle: string,
  rows: VPATRow[],
...
function truncateRemarkForTable(remarks: string): string {
