...
import { FontConfig, generateFontLoadScript } from "./font-config.js";
import { applyTextPresetScript } from "./typography-presets.js";
import { BlueprintNode, ComponentBlueprint } from "./component-templates.js";
 ...
 * on a new page.
 */
export function buildAllComponentsScript(
...
function buildNodeScript(
  ...
  if (node.layoutMode && node.layoutMode !== "NONE" && (node.kind === "frame" || isRoot)) {
    ...
    if (node.primaryAxisSizing) lines.push(`${nv}.primaryAxisSizingMode = ${JSON.stringify(node.primaryAxisSizing)};`);
    if (node.counterAxisSizing) lines.push(`${nv}.counterAxisSizingMode = ${JSON.stringify(node.counterAxisSizing)};`);
    if (node.primaryAxisAlign) lines.push(`${nv}.primaryAxisAlignItems = ${JSON.stringify(node.primaryAxisAlign)};`);
    if (node.counterAxisAlign) lines.push(`${nv}.counterAxisAlignItems = ${JSON.stringify(node.counterAxisAlign)};`);
  ...
  if (node.kind === "text") {
    if (node.textPreset) {
      lines.push(applyTextPresetScript(nv, node.textPreset, fontConfig));
    }
...
function getCreateFn(kind: BlueprintNode["kind"]): string {
  switch (kind) {
 ...
 */
function semanticToFallbackRgb(semantic: string): string {
  const s = semantic.toLowerCase();

  // Primary / brand actions
  if (s.includes("actions/primary/bg"))       return "{ r: 0.15, g: 0.39, b: 0.92 }";
  if (s.includes("actions/destructive"))      return "{ r: 0.86, g: 0.15, b: 0.15 }";
  if (s.includes("actions/secondary/bg"))     return "{ r: 0.95, g: 0.95, b: 0.97 }";
  if (s.includes("actions/secondary/border")) return "{ r: 0.80, g: 0.80, b: 0.85 }";

  // Surface
  if (s.includes("surface/subtle"))   return "{ r: 0.96, g: 0.96, b: 0.97 }";
  if (s.includes("surface/raised"))   return "{ r: 1, g: 1, b: 1 }";
  if (s.includes("surface/overlay"))  return "{ r: 1, g: 1, b: 1 }";
  if (s.includes("surface/inverse"))  return "{ r: 0.10, g: 0.10, b: 0.12 }";
  if (s.includes("surface/disabled")) return "{ r: 0.95, g: 0.95, b: 0.96 }";
  if (s.includes("surface/default"))  return "{ r: 1, g: 1, b: 1 }";

  // Text
  if (s.includes("text/on-color"))   return "{ r: 1, g: 1, b: 1 }";
  if (s.includes("text/inverse"))    return "{ r: 1, g: 1, b: 1 }";
  if (s.includes("text/primary"))    return "{ r: 0.07, g: 0.09, b: 0.15 }";
  if (s.includes("text/secondary"))  return "{ r: 0.37, g: 0.37, b: 0.42 }";
  if (s.includes("text/tertiary"))   return "{ r: 0.55, g: 0.55, b: 0.60 }";
  if (s.includes("text/disabled"))   return "{ r: 0.70, g: 0.70, b: 0.73 }";

  // Border
  if (s.includes("border/focus"))    return "{ r: 0.15, g: 0.39, b: 0.92 }";
  if (s.includes("border/strong"))   return "{ r: 0.70, g: 0.70, b: 0.73 }";
  if (s.includes("border/subtle"))   return "{ r: 0.92, g: 0.92, b: 0.94 }";
  if (s.includes("border/default"))  return "{ r: 0.82, g: 0.83, b: 0.86 }";

  // Field
  if (s.includes("field/bg"))        return "{ r: 1, g: 1, b: 1 }";
  if (s.includes("field/border"))    return "{ r: 0.80, g: 0.80, b: 0.85 }";

  // Feedback
  if (s.includes("feedback/success/bg"))   return "{ r: 0.87, g: 0.95, b: 0.90 }";
  if (s.includes("feedback/success/text")) return "{ r: 0.09, g: 0.55, b: 0.29 }";
  if (s.includes("feedback/warning/bg"))   return "{ r: 0.98, g: 0.93, b: 0.83 }";
  if (s.includes("feedback/warning/text")) return "{ r: 0.72, g: 0.49, b: 0.02 }";
  if (s.includes("feedback/danger/bg"))    return "{ r: 0.97, g: 0.88, b: 0.88 }";
  if (s.includes("feedback/danger/text"))  return "{ r: 0.73, g: 0.14, b: 0.14 }";
  if (s.includes("feedback/info/bg"))      return "{ r: 0.87, g: 0.92, b: 0.98 }";
  if (s.includes("feedback/info/text"))    return "{ r: 0.10, g: 0.36, b: 0.78 }";

  // Focus ring
  if (s.includes("focus/ring"))      return "{ r: 0.15, g: 0.39, b: 0.92 }";
 ...
 */
function buildTokenBindingBlock(
  blueprint: ComponentBlueprint,
  collectionId: string
): string {
  if (blueprint.tokenBindings.length === 0) return "";
  ...
  try {
    ...
    if (col) {
      ...
      ${blueprint.tokenBindings.map((tb) => {
        ...
        if (tb.property === "fills") return `bindFill(${targetExpr} || comp, ${varExpr});`;
        if (tb.property === "strokes") return `bindStroke(${targetExpr} || comp, ${varExpr});`;
        if (tb.property === "cornerRadius") return `bindRadius(${targetExpr} || comp, ${varExpr});`;
...
function buildBatchTokenBindingBlock(
  blueprints: ComponentBlueprint[],
  collectionId: string
): string {
  const bindableBlueprints = blueprints.filter((bp) => bp.tokenBindings.length > 0);
  if (bindableBlueprints.length === 0) return "";
