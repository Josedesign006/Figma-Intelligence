...
export interface CompressionOptions {
  ...
  maxSizeBytes?: number; // default 100KB for full
}
...
function determineTier(sizeBytes: number, maxFull?: number): CompressionTier {
  const fullLimit = maxFull ?? TIER_THRESHOLDS.full;
  if (sizeBytes < fullLimit) return "full";
  if (sizeBytes < TIER_THRESHOLDS.summary) return "summary";
  if (sizeBytes < TIER_THRESHOLDS.inventory) return "inventory";
  return "compact";
}
...
function summarizeNode(node: FigmaNode, depth: number, maxDepth: number): unknown {
  ...
  if (node.layoutMode !== undefined) base.layoutMode = node.layoutMode;
...
function inventoryNode(node: FigmaNode): unknown {
 ...
 */
export function compressNodeTree(
  node: FigmaNode,
  tier: CompressionTier = "summary",
): unknown {
  if (tier === "full") return node;
 ...
 */
export function compressTokenList(
  tokens: Token[],
  tier: CompressionTier = "summary",
): unknown {
  if (tier === "full") return tokens;
  ...
  if (tier === "inventory") {
    const byCollection = new Map<
      string,
      { collectionId: string; types: Set<string>; count: number }
    >();
 ...
 */
export function compressComponentSets(
  sets: ComponentSet[],
  tier: CompressionTier = "summary",
): unknown {
  if (tier === "full") return sets;
...
type DataShape = "nodeTree" | "tokenList" | "componentSetList" | "generic";

function detectShape(data: unknown): DataShape {
  if (data == null || typeof data !== "object") return "generic";
...
export function compressResponse(
  ...
  switch (shape) {
    case "nodeTree":
      compressed = compressNodeTree(data as FigmaNode, tier);
      break;

    case "tokenList":
      compressed = compressTokenList(data as Token[], tier);
      break;

    case "componentSetList": {
      // Handle both single set and array of sets
      const sets = Array.isArray(data)
        ? (data as ComponentSet[])
        : [data as ComponentSet];
...
function compressGeneric(data: unknown, tier: CompressionTier): unknown {
...
function trimDepth(data: unknown, depth: number, maxDepth: number): unknown {
  if (data == null || typeof data !== "object") return data;
