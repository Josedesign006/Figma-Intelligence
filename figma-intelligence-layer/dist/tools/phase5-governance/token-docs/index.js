...
exports.tokenDocsHandler = tokenDocsHandler;
...
function resolveAliasChain(variable, allVariablesById, maxDepth = 10) {
    const chain = [];
    ...
    while (depth < maxDepth) {
        ...
        if (firstModeValue &&
            ...
            const aliasId = firstModeValue.variableId;
...
async function fetchAndOrganizeTokens(args) {
    const bridge = await (0, figma_bridge_js_1.getBridge)();
    const raw = await bridge.getVariables(undefined, "full");
    ...
    const allVariablesById = new Map();
    ...
    for (const collection of collections) {
        ...
        for (const variable of collection.variables) {
            ...
            if (firstValue &&
                ...
                firstValue !== null &&
                "type" in firstValue &&
                firstValue.type === "VARIABLE_ALIAS") {
                const target = allVariablesById.get(firstValue.variableId);
                aliasOf = target?.name || firstValue.variableId;
            }
...
// ─── Renderers ──────────────────────────────────────────────────────────────
function renderMarkdown(categories, collections, allTokenCount, args) {
    const lines = [];
    ...
    for (const [, category] of categories) {
        ...
        for (const mode of modeNames)
            headerCols.push(mode);
        if (hasAlias)
            headerCols.push("Alias Of");
        if (args.includeAliasChains)
            headerCols.push("Alias Chain");
        headerCols.push("Description");
        lines.push(`| ${headerCols.join(" | ")} |`);
        lines.push(`| ${headerCols.map(() => "---").join(" | ")} |`);
        for (const token of category.tokens) {
            const row = [`\`${token.name}\``];
            for (const mode of modeNames) {
                row.push(formatValue(token.values[mode], token.resolvedType));
            }
            if (hasAlias)
                row.push(token.aliasOf ? `\`${token.aliasOf}\`` : "—");
            if (args.includeAliasChains) {
...
function renderJson(categories, collections, allTokenCount, _args) {
    const output = {
...
function renderHtml(categories, collections, allTokenCount, args) {
    const date = todayStamp();
...
function renderFigmaPluginCode(categories, _collections, _allTokenCount, args) {
...
// ─── Main Handler ───────────────────────────────────────────────────────────
async function tokenDocsHandler(args) {
    const notes = [];
