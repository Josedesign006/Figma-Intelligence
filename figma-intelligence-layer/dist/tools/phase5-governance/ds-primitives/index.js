...
exports.dsPrimitivesHandler = dsPrimitivesHandler;
...
function generateScale(name, baseHex, includeDark) {
...
function buildColorTokens(args) {
    const tokens = [
...
function buildTypographyTokens(fontConfig) {
    ...
    specs.push({ name: "typography/tracking/tight", resolvedType: "FLOAT", valuesByMode: { Base: -0.02 } }, { name: "typography/tracking/normal", resolvedType: "FLOAT", valuesByMode: { Base: 0 } }, { name: "typography/tracking/wide", resolvedType: "FLOAT", valuesByMode: { Base: 0.02 } });
    return specs;
}
async function ensureModes(bridge, collectionId, modeNames) {
    const collections = await bridge.getVariables(collectionId, "summary");
    const target = collections[0];
    if (!target)
        throw new Error(`Collection not found after creation: ${collectionId}`);
    const modes = Object.fromEntries(target.modes.map((mode) => [mode.name, mode.modeId]));
    for (const modeName of modeNames) {
        if (!modes[modeName]) {
            const created = await bridge.addMode(collectionId, modeName);
            modes[created.modeName] = created.modeId;
        }
    }
    return modes;
}
async function createCollectionWithTokens(bridge, name, initialModeName, tokens, extraModes = []) {
    const collection = await bridge.createVariableCollection(name, initialModeName);
    const modes = await ensureModes(bridge, collection.id, [initialModeName, ...extraModes]);
    await bridge.batchCreateVariables(tokens.map((token) => ({
        name: token.name,
        collectionId: collection.id,
        resolvedType: token.resolvedType,
        description: token.description,
        valuesByMode: Object.fromEntries(Object.entries(token.valuesByMode)
...
async function dsPrimitivesHandler(args) {
    ...
    const collectionsCreated = [
        await createCollectionWithTokens(bridge, `${args.brandName} Primitive Colors`, "Light", colorTokens, createDarkMode ? ["Dark"] : []),
