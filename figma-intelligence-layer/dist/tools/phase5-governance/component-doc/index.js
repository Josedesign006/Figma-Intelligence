...
function inferAnatomyDescription(name) {
    ...
    if (/label|title|heading/.test(n))
        return "Primary text label that communicates the purpose.";
    if (/text|body|description|subtitle/.test(n))
        return "Supporting text content providing additional context.";
    if (/image|thumbnail|avatar|photo/.test(n))
        return "Visual media element.";
    if (/container|wrapper|frame/.test(n))
        return "Layout container organizing child elements.";
    if (/divider|separator/.test(n))
        return "Visual separator between content sections.";
    if (/badge|indicator|dot/.test(n))
        return "Status or count indicator element.";
    if (/input|field/.test(n))
        return "Text input field for user data entry.";
    if (/button|btn|action|cta/.test(n))
        return "Interactive action element.";
    if (/background|bg/.test(n))
        return "Background layer providing visual foundation.";
    if (/border|stroke|outline/.test(n))
        return "Border or outline decoration.";
    if (/shadow|elevation/.test(n))
        return "Elevation or depth visual treatment.";
    if (/spacer/.test(n))
        return "Layout spacer for consistent spacing.";
    if (/close|dismiss|x/.test(n))
        return "Dismiss or close action control.";
...
function buildPurpose(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "Allows users to trigger a single, immediate action such as submitting a form, confirming a decision, or navigating to the next step.";
    if (/input|field|text.?field/.test(n))
        return "Provides a text entry point for users to input short-form data such as names, emails, or search queries.";
    if (/card/.test(n))
        return "Groups related content and actions into a visually distinct, scannable container.";
    if (/modal|dialog/.test(n))
        return "Presents focused content or decisions that require immediate user attention, blocking interaction with the page behind.";
    if (/toggle|switch/.test(n))
        return "Enables users to instantly turn a setting on or off without requiring a save action.";
    if (/checkbox/.test(n))
        return "Allows users to select one or more options from a set, typically within a form context.";
    if (/tab/.test(n))
        return "Organizes content into switchable panels within a single view, reducing page complexity.";
    if (/nav|menu|sidebar/.test(n))
        return "Provides the primary navigation structure for the application, helping users move between sections.";
    if (/toast|snackbar|notification/.test(n))
        return "Delivers brief, non-blocking feedback to users about the result of an action.";
    if (/avatar/.test(n))
        return "Represents a user or entity with a visual identifier, commonly used in profiles and comment threads.";
    if (/badge|chip|tag/.test(n))
        return "Communicates status, count, or categorical metadata in a compact visual form.";
    ...
    for (const [property, values] of Object.entries(snapshot.variantGroupProperties)) {
        if (/size|scale|density/i.test(property)) {
            for (const val of values) {
                ...
                if (/xs|extra.?small|tiny/.test(v)) {
                    useCase = "Dense data tables, compact toolbars.";
                    context = "Desktop-only, information-dense layouts.";
                    minTouchTarget = "24×24px minimum";
                }
                else if (/sm|small/.test(v)) {
                    useCase = "Secondary actions, inline controls.";
                    context = "Dense layouts, data tables, toolbars.";
                    minTouchTarget = "32×32px minimum";
                }
                else if (/md|medium|default|regular/.test(v)) {
                    useCase = "Primary UI interactions.";
                    context = "Most screens, forms, and content areas.";
                }
                else if (/lg|large/.test(v)) {
                    useCase = "Primary mobile actions, hero CTAs.";
                    context = "Mobile primary actions, prominent areas.";
                    minTouchTarget = "48×48px recommended";
                }
                else if (/xl|extra.?large/.test(v)) {
                    useCase = "Full-width mobile actions, hero sections.";
                    context = "Mobile-first layouts, high-priority actions.";
                    minTouchTarget = "56×56px recommended";
                }
...
function buildBehaviour(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "On click/tap, the button triggers its associated action immediately. During async operations, show a loading spinner and disable the button to prevent duplicate submissions. On keyboard Enter or Space, the same action fires. Long press has no additional behaviour.";
    if (/input|field|text.?field/.test(n))
        return "On focus, the field shows a focus ring and optional placeholder text fades. On input, real-time validation runs after a debounce period. On blur, full validation triggers and error messages appear inline. Tab moves to the next field; Shift+Tab moves backward.";
    if (/toggle|switch/.test(n))
        return "On click/tap, the toggle immediately switches state and applies the change without a save action. A brief transition animation indicates the state change. Keyboard Space toggles the value.";
    if (/modal|dialog/.test(n))
        return "Opens with a fade/scale transition. Focus is trapped inside the modal. Pressing Escape or clicking the backdrop dismisses it. On close, focus returns to the triggering element.";
    return "";
}
function buildInteractionRules(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "Keyboard: Enter and Space activate the button. Tab moves focus to the next focusable element. No focus trap. Pointer: click/tap triggers the action. Disabled state prevents all interaction and removes from tab order.";
    if (/input|field|text.?field/.test(n))
        return "Keyboard: Tab focuses the field; typing inserts text; Escape clears or reverts. Pointer: click focuses and places cursor. Focus ring must be visible at all times. Autocomplete: arrow keys navigate suggestions, Enter selects.";
    if (/modal|dialog/.test(n))
        return "Keyboard: Tab/Shift+Tab cycle within modal; Escape closes. Focus trap: focus must not leave the modal while open. On open, focus moves to the first focusable element or close button. On close, focus returns to the trigger.";
    if (/tab/.test(n))
        return "Keyboard: Arrow Left/Right moves between tabs; Home goes to first tab; End goes to last. Tab key moves focus into the tab panel content. Only the active tab is in the tab order.";
    return "";
}
function buildContentGuidance(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "Labels should be concise (1-3 words), action-oriented verbs or verb phrases in sentence case (e.g., 'Save changes', 'Delete item'). Avoid vague labels like 'Click here' or 'Submit'. Icons should reinforce the label meaning, not replace it. Truncation: labels must not truncate — resize the button or shorten the text.";
    if (/input|field|text.?field/.test(n))
        return "Labels must describe the expected input in sentence case. Placeholder text is optional help text — never a substitute for a label. Error messages should be specific ('Email must contain @') not generic ('Invalid input'). Helper text appears below the field and persists across states.";
    return "";
}
function buildResponsive(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "On mobile (< 768px), primary buttons should be full-width. On desktop, buttons use intrinsic width with minimum width of 120px. Button groups stack vertically on mobile. Touch targets expand to 48px minimum height on mobile.";
    if (/input|field|text.?field/.test(n))
        return "Fields stretch to fill container width at all breakpoints. On mobile, labels stack above fields. Field groups may stack vertically below 768px. Font size should be at least 16px on mobile to prevent iOS zoom.";
    if (/card/.test(n))
        return "Cards in a grid should reflow from multi-column to single-column below 768px. Card padding may reduce on smaller screens. Images within cards should maintain aspect ratio.";
    return "";
}
function buildImplementationNotes(snapshot) {
    const n = snapshot.name.toLowerCase();
    if (/button|btn|cta/.test(n))
        return "Use native <button> element, not <div> or <a>. For link-style navigation, use <a> styled as button. Support ref forwarding for parent focus management. Handle loading state with aria-busy='true'. Disabled buttons should use aria-disabled='true' rather than the disabled attribute if you need to keep them focusable for tooltip explanations.";
    if (/input|field|text.?field/.test(n))
        return "Use native <input> or <textarea>. Associate label with htmlFor/id. Error messages linked via aria-describedby. Support controlled and uncontrolled modes. Debounce onChange for validation (300ms recommended).";
    if (/modal|dialog/.test(n))
        return "Use native <dialog> element where supported. Implement focus trap with a sentinel approach. Restore focus on close. Prevent body scroll with overflow:hidden on body. Portal rendering recommended for z-index management.";
    return "";
}
