...
exports.buildAccessibilityAnnotationDocument = buildAccessibilityAnnotationDocument;
exports.formatDocumentReport = formatDocumentReport;
exports.resolveTargetNodeId = resolveTargetNodeId;
exports.captureSnapshot = captureSnapshot;
exports.createDocumentationPages = createDocumentationPages;
...
function inferStates(snapshot) {
    ...
    for (const prop of snapshot.componentProperties) {
        if (/state|status/i.test(prop.name)) {
            ...
            if (prop.value)
                discovered.add(prop.value);
        }
    }
    return Array.from(discovered);
}
function inferContentGuidance(snapshot) {
    const guidance = [];
...
function inferAccessibility(snapshot) {
...
function buildDocumentationGaps(snapshot, spec) {
    const gaps = [];
...
function buildSpec(snapshot, includeTokens) {
    const summaryParts = [
...
function formatSpecReport(spec) {
    const lines = [
...
function buildAnatomyUsageFallback(spec) {
    ...
    return {
        ...
        sections: [
            ...
            },
            {
                ...
                style: "bullets",
                items: variantSummary.concat(spec.states.length > 0
            ...
            },
            {
                ...
                style: "bullets",
                items: spec.implementationNotes.concat(spec.documentationGaps.length > 0
...
function buildAccessibilityFallback(spec) {
...
function collectDescendants(nodeId, childrenByParent) {
    const results = [];
...
function inferRole(node) {
    ...
    if (/(dialog|modal|sheet|drawer|popup)/.test(haystack))
        return "Dialog";
    if (/(combobox|typeahead|autocomplete)/.test(haystack))
        return "Combobox";
    if (/(dropdown|select|picker)\b/.test(haystack))
        return "Select";
    if (/(search|input|field|email|password|phone|amount|date|otp|textbox|text field)/.test(haystack))
        return "Text field";
    if (/(accordion|disclosure|expand|collapse)/.test(haystack))
        return "Accordion header";
    if (/\btab\b/.test(haystack))
        return "Tab";
    if (/(checkbox|check box)/.test(haystack))
        return "Checkbox";
    if (/(radio|option card|option set)/.test(haystack))
        return "Radio option";
    if (/(switch|toggle)/.test(haystack))
        return "Switch";
    if (/(slider|range)/.test(haystack))
        return "Slider";
    if (/(link|anchor|breadcrumb)/.test(haystack))
        return "Link";
    if (/\b(button|cta|submit|continue|next|back|cancel|save|apply|done|close)\b|place order\b|pay now\b/.test(haystack))
        return "Button";
...
function inferState(node) {
    ...
    const states = [];
    if (/(disabled)/.test(haystack))
        states.push("disabled");
    if (/(selected|active|checked|current)/.test(haystack))
        states.push("selected");
    if (/(expanded|open)/.test(haystack))
        states.push("expanded");
    if (/(collapsed|closed)/.test(haystack))
        states.push("collapsed");
    if (/(error|invalid)/.test(haystack))
        states.push("invalid");
    if (/(required)/.test(haystack))
...
function inferFocusableNode(node) {
    if (!node.visible || node.type === "TEXT")
        return undefined;
...
function inferPrimaryInteractionArea(snapshot) {
...
function buildAccessibilityAnnotationContext(snapshot) {
    const { nodes, byId, childrenByParent } = buildSnapshotIndex(snapshot);
    const area = inferPrimaryInteractionArea(snapshot);
    const relevantIds = new Set([area.areaNodeId, ...collectDescendants(area.areaNodeId, childrenByParent).map((node) => node.id)]);
...
function buildAccessibilityAnnotationDocument(snapshot, spec) {
...
function formatDocumentReport(document) {
    const lines = [
        document.title,
        "",
        document.summary,
...
async function generateDocumentWithAI(spec, document) {
            ...
            input: [
                {
                    ...
                    content: [
                        {
                            type: "input_text",
                            text: "You write design-system documentation. Return strict JSON only. Keep guidance concrete, implementation-aware, and grounded in the provided Figma-derived metadata.",
                        },
                    ],
                },
                {
...
async function buildGeneratedDocuments(snapshot, spec, documentType) {
    const seeds = [];
...
async function resolveTargetNodeId(args) {
    if (args.nodeId)
        return args.nodeId;
...
async function captureSnapshot(nodeId) {
    const bridge = await (0, figma_bridge_js_1.getBridge)();
    const runPart = async (label, code) => {
    ...
    const base = await runPart("base metadata", `
    ...
    `);
    const textLayers = await runPart("text layers", `
      var scannedNodes = [];
      if ("children" in node) {
        var queue = [node];
      ...
      return textLayers;
    `);
    const tokenAliases = await runPart("token aliases", `
    ...
    `);
    const componentShape = await runPart("component metadata", `
    ...
    `);
    const scanNodes = await runPart("interaction tree", `
...
async function generateSpecHandler(args) {
