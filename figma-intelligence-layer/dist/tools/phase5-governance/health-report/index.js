...
exports.healthReportHandler = healthReportHandler;
const figma_bridge_js_1 = require("../../../shared/figma-bridge.js");
const decision_log_js_1 = require("../../../shared/decision-log.js");
const index_js_1 = require("../../phase1-vision/a11y-audit/index.js");
const index_js_2 = require("../../phase2-accuracy/lint-rules/index.js");
const index_js_3 = require("../../phase2-accuracy/component-audit/index.js");
const WEIGHTS = {
    tokenCoverage: 0.25,
    accessibility: 0.25,
...
async function measureTokenCoverage() {
  ...
  `);
    if (!result.success || !result.result)
        return 70;
    const { total, tokenized } = result.result;
...
async function measureDocumentationCoverage() {
    ...
    if (components.length === 0)
        return 50;
    const documented = components.filter((c) => c.description && c.description.trim().length > 10).length;
    return Math.round((documented / components.length) * 100);
}
// ─── Weighted score calculation ───────────────────────────────────────────────
function computeOverall(scores) {
    return Math.round(scores.tokenCoverage * WEIGHTS.tokenCoverage +
...
// ─── Recommended actions ──────────────────────────────────────────────────────
function deriveTopActions(scores) {
    const actions = [
...
function formatTextReport(scores, topActions) {
    return [
        "╔══════════════════════════════════════════════════════╗",
...
function formatSlackDigest(scores, topActions) {
...
// ─── Figma page creation ──────────────────────────────────────────────────────
async function createHealthPage(scores, topActions) {
    const bridge = await (0, figma_bridge_js_1.getBridge)();
    const result = await bridge.execute(`
    (async () => {
  ...
  `);
    if (!result.success)
        throw new Error(`Health page creation failed: ${result.error}`);
    return result.result;
}
// ─── Main handler ─────────────────────────────────────────────────────────────
async function healthReportHandler(args) {
    const runAll = args.runAudits.includes("all");
    const shouldRun = (name) => runAll || args.runAudits.includes(name);
    ...
    if (shouldRun("tokens")) {
        try {
            tokenCoverage = await measureTokenCoverage();
        }
        catch { /* keep default */ }
    }
    if (shouldRun("a11y")) {
        try {
            ...
            if (rootResult.success && rootResult.result) {
                const a11y = await (0, index_js_1.a11yAuditHandler)({
                    nodeId: rootResult.result,
                    wcagLevel: "AA",
    ...
    if (shouldRun("components")) {
        try {
            const audit = await (0, index_js_3.componentAuditHandler)({ detectOrphans: true, groupBy: "component" });
    ...
    if (shouldRun("lint")) {
        try {
            const lintResult = await (0, index_js_2.lintRulesHandler)({
                action: "run",
                outputFormat: "ci",
                autoFix: false,
            });
