...
exports.designDecisionLogHandler = designDecisionLogHandler;
...
// ─── Handler ──────────────────────────────────────────────────────────────────
async function designDecisionLogHandler(args) {
    const { name, description, status = "Approved", category = "Design Research", pageName, screenCount, decisions, nearNodeId, } = args;
    if (!name)
        throw new Error("designDecisionLog: name is required.");
    if (!description)
        throw new Error("designDecisionLog: description is required.");
...
`;
    ...
    if (output.success && output.nodeId) {
        decision_log_js_1.decisionLog.log({
