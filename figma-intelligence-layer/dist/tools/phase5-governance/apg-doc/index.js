...
exports.guessPattern = guessPattern;
exports.shouldFallbackToGeneratedPage = shouldFallbackToGeneratedPage;
...
function collectSnapshotTerms(snapshot) {
    return [
        snapshot.name,
...
function guessPattern(snapshot, hint) {
...
function summarizeTarget(snapshot) {
...
function buildImplementationSnippet(pattern, framework, includeCodeExamples) {
    if (!includeCodeExamples)
        return undefined;
    return pattern.exampleSnippets[framework] || pattern.exampleSnippets.html;
}
function inferVariantA11yRules(snapshot, pattern) {
    const rules = [];
...
async function writeDescriptionToNode(nodeId, document, mode) {
    const bridge = await (0, figma_bridge_js_1.getBridge)();
    const report = (0, index_js_1.formatDocumentReport)(document);
...
function shouldFallbackToGeneratedPage(error) {
...
function buildApgDocument(snapshot, match, snippet) {
    ...
    const sections = [
        ...
        },
        {
            ...
            style: "bullets",
            items: pattern.accessibleName.concat(namingHints.length > 0 ? namingHints : ["No visible text was detected. Icon-only or abstract variants need an explicit accessible naming rule."]),
        },
        {
...
async function figmaApgDocHandler(args) {
    ...
    if (args.outputFormat === "figma-page" || args.outputFormat === "all") {
        figmaPageId = await (0, index_js_1.createDocumentationPages)([document], args.pageName || `APG Doc - ${snapshot.name}`);
    }
