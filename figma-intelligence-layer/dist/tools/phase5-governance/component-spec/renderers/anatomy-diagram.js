...
exports.computeMarkerPositions = computeMarkerPositions;
exports.renderAnatomyDiagram = renderAnatomyDiagram;
 ...
 */
function getDiagramElements(elements) {
 ...
 */
function computeMarkerPositions(compW, compH, cloneOffsetX, cloneOffsetY, elements) {
    const diagramElements = getDiagramElements(elements);
    const markers = [];
    // Buckets for each edge
    const edges = { top: [], bottom: [], left: [], right: [] };
    for (const el of diagramElements) {
        if (markers.length + edges.top.length + edges.bottom.length + edges.left.length + edges.right.length >= 26)
            break;
...
async function renderAnatomyDiagram(bridge, pageId, nodeId, anatomy, yPosition) {
    ...
    (async () => {
      ...
      // Legend entries
      var roleLabels = ${JSON.stringify(Object.fromEntries(["content-element", "optional-slot", "fixed-sub-component", "structural", "decorative"]
        .map(r => [r, roleLabel(r)])))};
...
//# sourceMappingURL=anatomy-diagram.js.map