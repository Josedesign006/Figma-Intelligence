...
exports.buildStateSpecsSection = buildStateSpecsSection;
function buildStateSpecsSection(data) {
    const { states, knowledge } = data;
    if (!knowledge || knowledge.stateSpecifications.length === 0)
        return null;
    const blocks = [];
    // Build a lookup from knowledge state entries
    const knowledgeLookup = new Map(knowledge.stateSpecifications.map((s) => [s.state.toLowerCase(), s]));
    ...
    if (states.states.length > 0) {
        ...
        for (const kState of knowledge.stateSpecifications) {
            const alreadyIncluded = detectedNames.some((n) => n.toLowerCase() === kState.state.toLowerCase());
            if (!alreadyIncluded) {
                rows.push({
