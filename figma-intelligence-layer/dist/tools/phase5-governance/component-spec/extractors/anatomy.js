...
exports.extractAnatomy = extractAnatomy;
/**
...
async function extractAnatomy(nodeId) {
...
function classifyRole(name, nodeType, visible, hasBoolean) {
    const n = name.toLowerCase();
    // Decorative elements — only truly decorative spacers/dividers
    if (/^(spacer|divider|separator|line|rule|bg|background)$/i.test(n))
        return "decorative";
    if (nodeType === "LINE" && /separator|divider/i.test(n))
        return "decorative";
    // Structural containers — only generic unnamed wrappers
    if (/^(frame|group)\s*\d*$/i.test(n))
        return "structural";
    if (/^(container|wrapper)$/i.test(n))
        return "structural";
    // Optional slots (controlled by boolean or typically optional)
    if (hasBoolean)
        return "optional-slot";
    if (/^(leading|trailing|prefix|suffix)/i.test(n))
        return "optional-slot";
    // Sub-components (instances)
    if (nodeType === "INSTANCE")
        return "fixed-sub-component";
