...
exports.toggleKnowledge = void 0;
exports.toggleKnowledge = {
    description: "Binary switch control | Immediate state change | On/Off preference toggle",
    stateSpecifications: [
        {
            state: "Off",
        ...
        {
            state: "On",
            visualChange: "Track fills with primary color; thumb slides to the right",
            opacity: "1",
            cursorWeb: "pointer",
        ...
        {
            state: "Disabled (off)",
            visualChange: "Track and thumb are muted; no pointer events; off position",
            opacity: "0.4",
        ...
        {
            state: "Disabled (on)",
            visualChange: "Track shows muted primary; thumb on right; no pointer events",
            opacity: "0.4",
            cursorWeb: "not-allowed",
    ...
    propertyDescriptions: {
        ...
        labelPosition: "Placement of the label relative to the toggle: 'start' (left/above) or 'end' (right/below)",
    ...
    interactionRules: [
        {
            ...
            action: "Toggle state flips immediately (on to off or off to on); onChange fires",
        },
        {
            event: "keydown (Space)",
        ...
        },
        {
            ...
            action: "Thumb follows the pointer; state commits when released past the midpoint",
        },
        {
            event: "change",
            trigger: "Toggle state changes by any method",
            action: "Effect takes place immediately — no form submission required",
        },
    ],
    contentGuidance: [
    ...
    responsiveBehaviour: [
        {
            breakpoint: "< 480px",
            behavior: "Toggle and label are placed on a single row; label may wrap to a second line if needed",
        },
        {
            breakpoint: "480px – 1024px",
            behavior: "Toggle and label sit inline within settings cards or form rows",
        },
        {
            breakpoint: "> 1024px",
            behavior: "Toggle may appear in dense settings panels; size stays consistent across breakpoints",
        },
    ],
    accessibilitySpec: {
        intro: "The toggle must use role=\"switch\" so assistive technology announces it as a binary on/off control with immediate effect.",
        requirements: [
            {
                requirement: "Element has role=\"switch\" with aria-checked reflecting the current state (true/false)",
                level: "A",
                notes: "WCAG 4.1.2 – Name, Role, Value",
            },
            {
                requirement: "An accessible name is provided via a visible label linked by htmlFor/id or aria-labelledby",
                level: "A",
                notes: "WCAG 1.3.1 – Info and Relationships",
            },
            {
                requirement: "State change is announced immediately by screen readers when toggled",
                level: "A",
            ...
            {
                requirement: "Track and thumb contrast meet 3:1 non-text UI requirement in both on and off states",
                level: "AA",
    ...
    qaAcceptanceCriteria: [
        {
            ...
            platform: "Web / Mobile",
            expectedResult: "Thumb slides smoothly between on and off positions; track color transitions within 150ms",
        },
        {
            check: "Disabled state",
            platform: "Web / Mobile",
            expectedResult: "Clicking or pressing Space on a disabled toggle has no effect; cursor shows not-allowed",
        },
        {
            check: "Keyboard operation",
            platform: "Web",
            expectedResult: "Space key toggles the state; Tab moves focus to/from the toggle",
        },
        {
            check: "Screen reader announcement",
            platform: "Web",
            expectedResult: "VoiceOver/NVDA reads label, role as 'switch', and current state ('on' or 'off')",
        },
        {
            check: "Drag behavior",
            platform: "Web / Mobile",
            expectedResult: "Dragging the thumb past the midpoint commits the state change; releasing before midpoint reverts",
        },
        {
            check: "Immediate effect",
            platform: "Web / Mobile",
            expectedResult: "Toggling fires the onChange callback immediately without waiting for form submission",
        },
    ],
    dos: [
