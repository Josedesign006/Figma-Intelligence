...
exports.translateCanonicalName = translateCanonicalName;
exports.tokenMigrateHandler = tokenMigrateHandler;
const figma_bridge_1 = require("../../../shared/figma-bridge");
const token_naming_1 = require("../../../shared/token-naming");
function translateCanonicalName(name, schema) {
    const analysis = (0, token_naming_1.analyzeTokenName)(name);
    ...
    const transforms = schema === "compact"
        ? [
            {
    ...
    for (const transform of transforms) {
        if (!transform.test.test(nextName))
            continue;
...
function buildVariableConflictMap(collections) {
    const map = new Map();
...
function planVariableItems(collections, schema) {
    ...
    for (const collection of collections) {
        for (const variable of collection.variables) {
            ...
            const conflictReasons = conflictId && conflictId !== variable.id
...
function planStyleItems(styles, schema) {
    const items = [];
    const groups = [
        ["paint", styles.paint ?? []],
        ["text", styles.text ?? []],
...
async function renameStyles(styleItems) {
    if (styleItems.length === 0)
        return [];
    const bridge = await (0, figma_bridge_1.getBridge)();
...
async function createVariableAliases(collections, items) {
    const bridge = await (0, figma_bridge_1.getBridge)();
    const collectionMap = new Map(collections.map((collection) => [collection.id, collection]));
    const aliases = [];
    for (const item of items) {
        if (item.kind !== "variable" || !item.collectionId)
            continue;
        const collection = collectionMap.get(item.collectionId);
        if (!collection || !item.suggestedName)
            continue;
        const variable = collection.variables.find((entry) => entry.id === item.id);
        if (!variable)
            continue;
        const valuesByMode = Object.fromEntries(collection.modes.map((mode) => [
            mode.modeId,
            { type: "VARIABLE_ALIAS", variableId: item.id },
        ]));
        const created = await bridge.createVariable(item.originalName, collection.id, variable.type, valuesByMode, `Alias created by figma_token_migrate for ${item.suggestedName}`);
        aliases.push({
            kind: "variable-alias",
            id: created.id,
...
async function tokenMigrateHandler(args) {
    ...
    const bridge = await (0, figma_bridge_1.getBridge)();
    const collections = scope === "styles"
        ? []
        : (await bridge.getVariables(undefined, "full")).filter((collection) => !args.collectionName || collection.name === args.collectionName);
    const styles = scope === "variables"
        ? {}
        : await bridge.getStyles();
