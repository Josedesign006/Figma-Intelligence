...
exports.renderAnnotationPage = renderAnnotationPage;
...
`;
// ─── Design Tokens ──────────────────────────────────────────────────────────
const MK = a11y_annotation_kit_js_1.STAMP_SPECS;
const MAX_LEFT_COL_W = 620;
// ─── Main Renderer ──────────────────────────────────────────────────────────
async function renderAnnotationPage(bridge, opts) {
...
async function renderRightColumn(bridge, contentRowId, tableRows, implNotes) {
  ...
  `);
    if (!initRight.success)
        throw new Error(`Right column init failed: ${initRight.error}`);
    const { rightColId, tblId } = initRight.result;
...
//# sourceMappingURL=a11y-annotate-renderer.js.map