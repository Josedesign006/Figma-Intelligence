...
exports.renderKeyboardSrOrderPage = renderKeyboardSrOrderPage;
// ── Design Tokens ────────────────────────────────────────────────────────────
const PAGE_W = 1200;
const PAD = 50;
...
function sectionHeadingJS(title, parent = "m") {
    return `(function(){
...
// ── Main Renderer ────────────────────────────────────────────────────────────
async function renderKeyboardSrOrderPage(bridge, analysis, customPageName) {
  ...
  `);
    if (!initResult.success)
        throw new Error(`KSR init: ${initResult.error}`);
...
// ── Section Renderers ───────────────────────────────────────────────────────
async function renderHeader(bridge, mId, analysis) {
...
async function renderScopeAndAssumptions(bridge, mId, analysis) {
...
async function renderTabOrderTable(bridge, mId, tabOrder) {
    const BATCH_SIZE = 15;
...
async function renderReadingOrder(bridge, mId, readingOrder) {
...
async function renderInteractionAnnouncements(bridge, mId, analysis) {
    ...
    const stateRows = analysis.interactionAnnouncements.stateChanges.map((s) => ({
...
async function renderFocusManagement(bridge, mId, rules) {
...
async function renderImplementationNotes(bridge, mId, analysis) {
...
async function renderWarnings(bridge, mId, warnings) {
...
async function renderAuditSummary(bridge, mId, summary) {
...
//# sourceMappingURL=keyboard-sr-order-figma-page.js.map