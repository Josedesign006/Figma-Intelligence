...
exports.MIN_TARGET_SIZE_AA = exports.MIN_TOUCH_TARGET = void 0;
...
exports.checkTargetSizeMinimum = checkTargetSizeMinimum;
exports.checkNonTextContent = checkNonTextContent;
exports.checkInfoAndRelationships = checkInfoAndRelationships;
exports.checkMeaningfulSequence = checkMeaningfulSequence;
exports.checkUseOfColor = checkUseOfColor;
exports.checkNonTextContrast = checkNonTextContrast;
exports.checkTextSpacing = checkTextSpacing;
exports.checkReflow = checkReflow;
exports.checkImagesOfText = checkImagesOfText;
exports.checkContentOnHover = checkContentOnHover;
exports.checkFocusOrder = checkFocusOrder;
exports.checkLinkPurpose = checkLinkPurpose;
exports.checkHeadingsAndLabels = checkHeadingsAndLabels;
exports.checkFocusNotObscured = checkFocusNotObscured;
exports.checkErrorIdentification = checkErrorIdentification;
exports.checkLabelsOrInstructions = checkLabelsOrInstructions;
exports.checkNameRoleValue = checkNameRoleValue;
exports.checkOrientation = checkOrientation;
exports.checkIdentifyInputPurpose = checkIdentifyInputPurpose;
...
exports.MIN_TARGET_SIZE_AA = 24;
function checkTargetSizeMinimum(node) {
    if (!node.absoluteBoundingBox)
        return null;
    const w = node.width ?? 0;
    const h = node.height ?? 0;
    if (w < exports.MIN_TARGET_SIZE_AA || h < exports.MIN_TARGET_SIZE_AA) {
        return {
            severity: "error",
            criterion: "2.5.8",
            nodeId: node.id,
            nodeName: node.name,
            issue: `Interactive element is ${w}×${h}px, below the minimum ${exports.MIN_TARGET_SIZE_AA}×${exports.MIN_TARGET_SIZE_AA}px target size`,
            currentValue: `${w}×${h}px`,
            suggestedFix: `Resize to at least ${exports.MIN_TARGET_SIZE_AA}×${exports.MIN_TARGET_SIZE_AA}px`,
            autoFixAvailable: true,
        };
    }
    return null;
...
function checkNonTextContent(node, allNodes) {
    // Check IMAGE fill nodes and VECTOR nodes
    const isImage = node.fills?.some((f) => f.type === "IMAGE") ||
        node.type === "VECTOR" ||
    ...
    if (parent?.children) {
        const hasSiblingLabel = parent.children.some((c) => c.type === "TEXT" && c.id !== node.id);
        if (hasSiblingLabel)
            return null;
    }
    return {
...
function checkInfoAndRelationships(textNodes) {
    const issues = [];
...
function checkMeaningfulSequence(allNodes) {
    const issues = [];
    ...
    for (const node of allNodes) {
        ...
        const visualOrder = [...childrenWithPos].sort((a, b) => a.visualY - b.visualY || a.visualX - b.visualX);
...
function checkUseOfColor(componentSetNodes) {
    const issues = [];
    for (const set of componentSetNodes) {
        if (!set.children || set.children.length < 2)
            continue;
        ...
        for (let i = 0; i < set.children.length - 1; i++) {
            ...
            if (issues.some((i) => i.nodeId === set.id))
                break;
        }
    }
    return issues;
...
function checkNonTextContrast(node, allNodes) {
    // Check strokes on interactive/UI elements
    if (!node.strokes || node.strokes.length === 0)
        return null;
    const strokeColor = node.strokes.find((s) => s.type === "SOLID" && s.color)?.color;
    if (!strokeColor)
        return null;
    const bgColor = findParentBgColor(node.id, allNodes);
    if (!bgColor)
        return null;
    const strokeHex = (0, token_utils_js_1.figmaRgbaToHex)(strokeColor.r, strokeColor.g, strokeColor.b);
    const bgHex = (0, token_utils_js_1.figmaRgbaToHex)(bgColor.r, bgColor.g, bgColor.b);
    const ratio = (0, token_utils_js_1.computeContrastRatio)(strokeHex, bgHex);
...
function checkTextSpacing(textNode) {
...
function checkReflow(frameNodes) {
    const issues = [];
    for (const frame of frameNodes) {
        ...
        const isFixedWidth = frame.counterAxisSizingMode === "FIXED" ||
...
function checkImagesOfText(allNodes) {
    const issues = [];
...
function checkContentOnHover(componentSetNodes) {
    const issues = [];
    for (const set of componentSetNodes) {
        if (!set.children)
            continue;
...
function checkFocusOrder(interactiveNodes) {
    const issues = [];
    ...
    const visualOrder = [...withPos].sort((a, b) => a.y - b.y || a.x - b.x);
    let mismatches = 0;
    for (let i = 0; i < visualOrder.length; i++) {
...
function checkLinkPurpose(interactiveNodes) {
    const issues = [];
    for (const node of interactiveNodes) {
        const name = node.name.toLowerCase();
        const isLink = name.includes("link") ||
...
function checkHeadingsAndLabels(frameNodes) {
    const issues = [];
    for (const frame of frameNodes) {
...
function checkFocusNotObscured(componentSetNodes) {
    const issues = [];
    for (const set of componentSetNodes) {
        if (!set.children)
            continue;
        ...
        if (!focusVariant || !focusVariant.children)
            continue;
...
function checkErrorIdentification(componentSetNodes) {
    const issues = [];
    for (const set of componentSetNodes) {
        if (!set.children)
            continue;
        ...
        const hasErrorDescription = errorText &&
            errorText.length > 3 &&
...
function checkLabelsOrInstructions(interactiveNodes, allNodes) {
    const issues = [];
    ...
    for (const node of interactiveNodes) {
        ...
        const hasInternalLabel = node.children?.some((c) => c.type === "TEXT" &&
            c.characters &&
        ...
        const parent = findParentNode(node.id, allNodes);
        const hasSiblingLabel = parent?.children?.some((c) => c.type === "TEXT" &&
            c.id !== node.id &&
            c.characters &&
            c.characters.trim().length > 0 &&
            !c.name.toLowerCase().includes("placeholder"));
...
function checkNameRoleValue(interactiveNodes) {
    const issues = [];
    for (const node of interactiveNodes) {
...
function checkOrientation(componentSetNodes) {
...
function checkIdentifyInputPurpose(interactiveNodes) {
    const issues = [];
    const purposeHints = [
    ...
    for (const node of interactiveNodes) {
        ...
        const matchesPurpose = purposeHints.some((hint) => name.includes(hint) || (node.description ?? "").toLowerCase().includes(hint));
...
function findParentNode(nodeId, allNodes) {
...
function hasStructuralDifference(a, b) {
    ...
    for (let i = 0; i < aChildren.length; i++) {
        if (aChildren[i].type !== bChildren[i]?.type)
            return true;
    }
