...
exports.imageBufferToPngDataUri = imageBufferToPngDataUri;
exports.resolveImage = resolveImage;
exports.screenClonerHandler = screenClonerHandler;
...
const design_system_context_js_1 = require("../../../shared/design-system-context.js");
...
const font_matcher_js_1 = require("./font-matcher.js");
const icon_resolver_js_1 = require("./icon-resolver.js");
const icon_catalog_js_1 = require("../../../shared/icon-catalog.js");
const CONFIDENCE_THRESHOLD = 0.75;
async function imageBufferToPngDataUri(buffer) {
    const normalizedBuffer = await (0, sharp_1.default)(buffer).png().toBuffer();
    return `data:image/png;base64,${normalizedBuffer.toString("base64")}`;
}
function clampPercentage(value) {
    return Math.min(100, Math.max(0, value));
}
function snapToGrid(value, grid = 4, minimum = 1) {
...
async function getImageDimensions(image) {
    ...
    const metadata = await (0, sharp_1.default)(Buffer.from(base64, "base64")).metadata();
    if (!metadata.width || !metadata.height) {
        throw new Error("screenClonerHandler: Unable to determine source image dimensions");
    }
    return { width: metadata.width, height: metadata.height };
}
function pctToPx(value, total) {
...
function normalizeIconName(value) {
    ...
    const catalogResult = (0, icon_catalog_js_1.searchIcons)(normalized, { limit: 1 });
...
async function fetchOpenSourceIconSvg(manifest) {
    ...
    const catalogResult = (0, icon_catalog_js_1.searchIcons)(iconName, { limit: 1 });
    let prefix;
    let slug;
...
async function extractZoneFallbackImage(image, node, sourceDimensions) {
    const base64 = image.replace(/^data:image\/\w+;base64,/, "");
    const buffer = Buffer.from(base64, "base64");
    const left = Math.max(0, Math.min(sourceDimensions.width - 1, Math.round((clampPercentage(node.boundingBox.x) / 100) * sourceDimensions.width)));
    const top = Math.max(0, Math.min(sourceDimensions.height - 1, Math.round((clampPercentage(node.boundingBox.y) / 100) * sourceDimensions.height)));
    const width = Math.max(1, Math.min(sourceDimensions.width - left, Math.round((clampPercentage(node.boundingBox.width) / 100) * sourceDimensions.width)));
    const height = Math.max(1, Math.min(sourceDimensions.height - top, Math.round((clampPercentage(node.boundingBox.height) / 100) * sourceDimensions.height)));
...
async function populateFallbackCrop(bridge, image, match, sourceDimensions, frameWidth) {
    ...
    match.fallbackHeight = snapToGrid(Math.max(1, Math.round((frameWidth * fallback.cropHeight) / fallback.cropWidth)));
    match.fallbackWidth = snapToGrid(Math.max(16, Math.round((frameWidth * fallback.cropWidth) / sourceDimensions.width)));
}
function flattenLeafNodes(nodes) {
    return nodes.flatMap((node) => node.children && node.children.length > 0 ? flattenLeafNodes(node.children) : [node]);
}
...
function averageGap(children, direction) {
    if (children.length < 2 || direction === "none")
        return 0;
    const ordered = [...children].sort((a, b) => direction === "horizontal"
        ? a.boundingBox.x - b.boundingBox.x
        : a.boundingBox.y - b.boundingBox.y);
    const gaps = ordered
        .slice(1)
        .map((child, index) => direction === "horizontal"
...
function inferPadding(node) {
    if (!node.children || node.children.length === 0)
        return undefined;
    const left = Math.max(0, Math.min(...node.children.map((child) => child.boundingBox.x - node.boundingBox.x)));
    const top = Math.max(0, Math.min(...node.children.map((child) => child.boundingBox.y - node.boundingBox.y)));
    const right = Math.max(0, Math.min(...node.children.map((child) => node.boundingBox.x +
        node.boundingBox.width -
        (child.boundingBox.x + child.boundingBox.width))));
    const bottom = Math.max(0, Math.min(...node.children.map((child) => node.boundingBox.y +
        node.boundingBox.height -
        (child.boundingBox.y + child.boundingBox.height))));
    return { top, right, bottom, left };
}
function defaultGapForType(layoutType) {
    switch (layoutType) {
        case "header":
        case "toolbar":
        case "button-group":
...
function defaultPaddingForType(layoutType) {
    switch (layoutType) {
        case "header":
        case "toolbar":
...
function defaultCrossAlignment(layoutType, direction) {
...
function enrichLayout(node) {
    ...
    return {
        ...
        layout: {
            ...
            stackingOrder: node.layout?.stackingOrder ??
                (node.siblingHints?.overlay || node.layoutType === "overlay" ? "overlay" : "normal"),
        },
    };
}
function ensurePageRoot(nodes) {
...
function buildResolvedTree(nodes, matchesById) {
...
function sortResolvedChildren(children, direction) {
...
function serializeResolvedTree(nodes, frameWidth, frameHeight, parentNode) {
    return nodes.map((resolved) => {
        ...
        const absolute = node.siblingHints?.overlay === true ||
        ...
        const relativeX = parentNode
            ? snapToGrid(pctToPx(node.boundingBox.x, frameWidth) - pctToPx(parentNode.boundingBox.x, frameWidth))
            : x;
        const relativeY = parentNode
            ? snapToGrid(pctToPx(node.boundingBox.y, frameHeight) - pctToPx(parentNode.boundingBox.y, frameHeight))
            : y;
        return {
            id: node.id,
            label: node.label,
            ...
            renderStrategy: sortedChildren.length > 0 ? "container" : match?.renderStrategy ?? "crop",
...
function buildFrameScript(frameWidth, frameHeight, tree, frameName, targetPage, annotateUnmatched) {
    return `
...
function buildPixelCloneScript(imageHash, frameWidth, frameHeight, frameName, targetPage) {
    return `
  const frameName = ${JSON.stringify(frameName)};
  const targetPageName = ${JSON.stringify(targetPage ?? null)};
  const frameWidth = ${frameWidth};
...
async function screenClonerHandler(args) {
    const { image, cloneMode, frameWidth = 1440, targetPage, annotateUnmatched = false, } = args;
    ...
    const frameHeight = Math.max(1, Math.round((frameWidth * sourceDimensions.height) / sourceDimensions.width));
    if (cloneMode === "pixel") {
        const importedImage = await bridge.importImage(resolvedImage);
        const pixelScript = buildPixelCloneScript(importedImage.imageHash, frameWidth, frameHeight, frameName, targetPage);
    ...
    const manifests = await Promise.all(leafNodes.map((node) => vision.identify(node.zoneImage ?? resolvedImage)));
    const componentSets = await bridge.getComponentSets();
    const styles = await bridge.getStyles();
    const designSystemContext = await (await (0, design_system_context_js_1.getDesignSystemContextStore)(bridge)).hydrate();
    const matches = leafNodes.map((node, index) => {
        const manifest = manifests[index];
        const effectiveThreshold = cloneMode === "system" ? 0.85 : CONFIDENCE_THRESHOLD;
        const { component, confidence } = (0, design_system_matcher_js_1.matchComponentInContext)((0, design_system_matcher_js_1.manifestToDSMatchRequest)(manifest), designSystemContext, effectiveThreshold);
    ...
    await Promise.all(matches.map(async (match) => {
        await populateFallbackCrop(bridge, resolvedImage, match, sourceDimensions, frameWidth);
        match.fontStyleMatch = (0, font_matcher_js_1.resolveFontStyleMatch)(match.manifest, styles);
        match.iconComponentMatch = (0, icon_resolver_js_1.resolveIconComponentMatch)(match.manifest, componentSets);
        match.openSourceIconSvg = await fetchOpenSourceIconSvg(match.manifest);
        if (match.dsComponent !== null)
            return;
        const iconOnly = match.manifest.iconPresent &&
            !match.manifest.textContent &&
            (match.iconComponentMatch !== null || match.openSourceIconSvg !== null);
        const textOnly = Boolean(match.manifest.textContent) &&
            !match.manifest.interactiveElement &&
            match.fontStyleMatch !== null;
        if (iconOnly) {
            match.renderStrategy = "icon";
            match.fallbackHeight = snapToGrid(Math.max(16, Math.round((frameHeight * clampPercentage(match.node.boundingBox.height)) / 100)));
            match.fallbackWidth = snapToGrid(Math.max(16, Math.round((frameWidth * clampPercentage(match.node.boundingBox.width)) / 100)));
            return;
        }
        if (textOnly) {
            match.renderStrategy = "text";
            match.fallbackHeight = snapToGrid(Math.max(match.fontStyleMatch?.fontSize ?? 16, Math.round((frameHeight * clampPercentage(match.node.boundingBox.height)) / 100)));
            match.fallbackWidth = snapToGrid(Math.max(16, Math.round((frameWidth * clampPercentage(match.node.boundingBox.width)) / 100)));
            return;
        }
    ...
    const script = buildFrameScript(frameWidth, frameHeight, serializedTree, frameName, targetPage, annotateUnmatched);
    ...
    const unmatchedComponents = matches
        ...
        .map((match) => `${match.node.label} (${match.manifest.componentType}, confidence: ${match.confidence.toFixed(2)})`);
    const totalNodes = countNodes(segmentedTree);
    const tokenCoverage = componentSets.length > 0
