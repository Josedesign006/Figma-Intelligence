...
exports.webhookListenerHandler = webhookListenerHandler;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const decision_log_js_1 = require("../../../shared/decision-log.js");
// ─── Persistence ──────────────────────────────────────────────────────────────
const SUBSCRIPTIONS_PATH = process.env.WEBHOOK_SUBSCRIPTIONS_PATH ||
    path_1.default.join(process.cwd(), ".figma-webhooks.json");
async function readSubscriptions() {
    try {
        const data = await promises_1.default.readFile(SUBSCRIPTIONS_PATH, "utf-8");
...
async function writeSubscriptions(subs) {
    await promises_1.default.writeFile(SUBSCRIPTIONS_PATH, JSON.stringify(subs, null, 2), "utf-8");
}
...
function figmaToken() {
    ...
    if (!token)
        throw new Error("webhookListener: FIGMA_ACCESS_TOKEN environment variable is required.");
    return token;
}
...
async function figmaPost(path_, body) {
...
async function figmaDelete(path_) {
    const response = await fetch(`${FIGMA_API_BASE}${path_}`, {
        method: "DELETE",
...
async function postSlackNotification(webhookUrl, message, fields) {
    try {
        const body = {
            text: message,
            attachments: [
...
async function postGitHubComment(repoSlug, body) {
...
async function handleWebhookEvent(event, subscription) {
    const results = {
    ...
    const isTokenChange = event.event_type === "FILE_UPDATE" &&
        event.description?.toLowerCase().includes("token");
    const isComponentChange = event.event_type === "FILE_UPDATE" &&
        !isTokenChange;
    ...
    if (isTokenChange && triggers?.onTokenChange) {
        ...
        if ((trigger === "notify-slack" || trigger === "all") && notificationChannels?.slack) {
            const sent = await postSlackNotification(notificationChannels.slack, `Figma Token Change Detected in ${event.file_name ?? event.file_key}`, [
    ...
    if (isComponentChange && triggers?.onComponentChange) {
        ...
        if ((trigger === "notify-slack" || trigger === "all") && notificationChannels?.slack) {
            const sent = await postSlackNotification(notificationChannels.slack, `Figma Component Change Detected in ${event.file_name ?? event.file_key}`, [
                { title: "File", value: event.file_name ?? event.file_key ?? "Unknown" },
                { title: "Time", value: event.timestamp ?? new Date().toISOString() },
    ...
    if (isLibraryPublish && triggers?.onLibraryPublish) {
        ...
        if (trigger === "notify-all" && notificationChannels?.slack) {
            const sent = await postSlackNotification(notificationChannels.slack, `Figma Library Published: ${event.file_name ?? event.file_key}`, [
                { title: "File", value: event.file_name ?? event.file_key ?? "Unknown" },
...
async function actionSubscribe(args) {
    ...
    const webhookId = String(responseData["id"] ??
        responseData["webhook_id"] ??
        `local_${Date.now()}`);
    // If multiple events, register additional webhooks
    const additionalIds = [webhookId];
    for (let i = 1; i < events.length; i++) {
        try {
            ...
            const extraId = String(extra["id"] ?? `local_${Date.now()}_${i}`);
...
async function actionUnsubscribe(args) {
    const existing = await readSubscriptions();
    ...
    try {
        ...
        if (teamId) {
            ...
            figmaWebhooks = rawHooks
                ...
                fileKey: args.fileKey,
                events: [String(h["event_type"] ?? "FILE_UPDATE")],
                triggers: undefined,
...
// ─── Main handler ─────────────────────────────────────────────────────────────
async function webhookListenerHandler(args) {
    switch (args.action) {
        case "subscribe":
