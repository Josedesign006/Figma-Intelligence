...
exports.watchDocsHandler = watchDocsHandler;
...
function defaultSnapshotDir() {
    return path_1.default.join(process.cwd(), ".figma-docs-snapshots");
}
async function ensureSnapshotDir(dir) {
    try {
        await promises_1.default.mkdir(dir, { recursive: true });
    }
    catch { /* exists */ }
}
async function loadSnapshot(dir, nodeId) {
    const filePath = path_1.default.join(dir, `${nodeId.replace(/[:/]/g, "_")}.json`);
    try {
...
async function saveSnapshot(dir, snapshot) {
    const filePath = path_1.default.join(dir, `${snapshot.nodeId.replace(/[:/]/g, "_")}.json`);
...
// ─── Component scanning ───────────────────────────────────────────────────
async function scanComponents(nodeIds) {
    ...
    const rawSnapshots = result.result;
...
// ─── Changelog generator ──────────────────────────────────────────────────
function generateChangelog(drifts) {
...
function renderChangelogMarkdown(entries) {
    const lines = [];
...
// ─── Main handler ──────────────────────────────────────────────────────────
async function watchDocsHandler(args) {
    ...
    switch (args.action) {
        case "check": {
            ...
            if (args.autoRegenerate && staleComponents.length > 0) {
                for (const snap of current) {
                    if (!staleComponents.includes(snap.name))
                        continue;
                    try {
                        await (0, index_js_1.componentSpecHandler)({
                            nodeId: snap.nodeId,
