...
exports.handoffSpecHandler = handoffSpecHandler;
const figma_bridge_js_1 = require("../../../shared/figma-bridge.js");
// ─── Figma extraction ──────────────────────────────────────────────────────
async function extractNodeTree(nodeId, maxDepth) {
...
function generateElementCss(el, unit, remBase) {
    const u = (val) => {
        if (val === 0)
            return "0";
    ...
    if (el.spacing.layoutMode !== "NONE") {
        ...
        if (el.spacing.itemSpacing > 0)
            lines.push(`  gap: ${u(el.spacing.itemSpacing)};`);
    }
    // Dimensions
...
// ─── Markdown renderer ─────────────────────────────────────────────────────
function renderMarkdown(rootName, rootBounds, elements, redlines, assets, args) {
    const lines = [];
    ...
    for (const el of elements) {
        if (el.depth > 3)
            continue; // Skip deeply nested for readability
        ...
        if (el.typography) {
            ...
            if (t.letterSpacing)
                lines.push(`${indent}| Letter Spacing | ${t.letterSpacing}px |`);
        }
        lines.push(``);
    }
    // Redlines
...
// ─── Figma page renderer ───────────────────────────────────────────────────
async function createHandoffPage(nodeId, rootName, elements, redlines) {
    const bridge = await (0, figma_bridge_js_1.getBridge)();
    const result = await bridge.execute(`
    (async () => {
      ...
      if (sourceNode) {
        ...
        // Token entries
        var tokenEntries = ${JSON.stringify(elements.flatMap((el) => el.tokens).filter((t, i, arr) => arr.findIndex((o) => o.tokenName === t.tokenName) === i).slice(0, 40))};
...
// ─── Main handler ──────────────────────────────────────────────────────────
async function handoffSpecHandler(args) {
    const maxDepth = args.maxDepth ?? 6;
    const cssUnit = args.cssUnit ?? "px";
    const remBase = args.remBase ?? 16;
    const { elements, assets, redlines, rootName, rootBounds } = await extractNodeTree(args.nodeId, maxDepth);
