...
exports.iconLibrarySyncHandler = iconLibrarySyncHandler;
const figma_bridge_js_1 = require("../../../shared/figma-bridge.js");
// ─── Helpers ───────────────────────────────────────────────────────────────
function cleanIconName(name, prefix) {
    let clean = name;
...
function iconToPascalCase(name) {
    return name
        .split("-")
        .filter(Boolean)
...
function iconToCamelCase(name) {
    const pascal = iconToPascalCase(name);
    return pascal.charAt(0).toLowerCase() + pascal.slice(1);
}
function categorizeIcon(name) {
    const lower = name.toLowerCase();
    if (/arrow|chevron|caret|expand|collapse|direction/.test(lower))
        return "navigation";
    if (/edit|delete|add|remove|create|copy|paste|cut|save/.test(lower))
        return "action";
    if (/check|close|error|warning|info|success|alert/.test(lower))
        return "status";
    if (/user|person|people|group|account|avatar/.test(lower))
        return "user";
    if (/file|folder|document|upload|download|attachment/.test(lower))
        return "file";
    if (/search|filter|sort|zoom/.test(lower))
        return "search";
    if (/settings|gear|cog|tune|config/.test(lower))
        return "settings";
    if (/home|menu|grid|list|layout|dashboard/.test(lower))
        return "layout";
    if (/mail|email|message|chat|notification|bell/.test(lower))
        return "communication";
    if (/lock|unlock|security|shield|key/.test(lower))
        return "security";
    if (/calendar|clock|time|date|schedule/.test(lower))
        return "time";
    if (/star|heart|like|favorite|bookmark/.test(lower))
        return "social";
    if (/link|share|external|globe|world/.test(lower))
        return "connectivity";
    return "misc";
}
// ─── Figma icon extraction ─────────────────────────────────────────────────
async function extractFigmaIcons(sourceNodeId, namePrefix, exportSize) {
...
function generateSvelteIcon(icon) {
    let svg = icon.svgContent
        .replace(/width="[^"]*"/, 'width={size}')
        .replace(/height="[^"]*"/, 'height={size}')
        .replace(/fill="(?!none)[^"]*"/g, 'fill={color}')
...
function generateIconCatalog(icons, framework) {
...
// ─── Main handler ──────────────────────────────────────────────────────────
async function iconLibrarySyncHandler(args) {
