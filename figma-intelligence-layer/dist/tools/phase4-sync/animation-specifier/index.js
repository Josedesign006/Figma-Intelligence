...
exports.animationSpecifierHandler = animationSpecifierHandler;
...
function detectEasing(action) {
    ...
    if (typeof easing === "string")
        return easing;
    const type = easing["type"]?.toUpperCase();
    if (!type)
        return "EASE_IN_OUT";
    if (type.includes("SPRING") || type === "SPRING_PRESET")
        return "SPRING";
    if (type.includes("EASE_IN_AND_OUT") || type.includes("EASE_IN_OUT"))
        return "EASE_IN_OUT";
    if (type.includes("EASE_IN"))
        return "EASE_IN";
    if (type.includes("EASE_OUT"))
        return "EASE_OUT";
    if (type.includes("LINEAR"))
        return "LINEAR";
    return "EASE_IN_OUT";
}
function detectTransitionType(action) {
    const transition = action["transition"];
    if (!transition)
        return { type: "DISSOLVE", direction: "none" };
    const rawType = transition["type"]?.toUpperCase() ?? "DISSOLVE";
    const direction = transition["direction"]?.toUpperCase() ?? "none";
    return { type: rawType, direction };
}
function parseConnections(connections) {
    const specs = [];
    for (const conn of connections) {
        const transition = conn.action["transition"];
        if (!transition)
            continue;
        const durationRaw = transition["duration"];
...
// ─── Code generators ──────────────────────────────────────────────────────────
function getSlideAxis(type, direction) {
...
function generateCSS(specs) {
    ...
    for (const spec of specs) {
        ...
        else if (type === "PUSH" || type === "SLIDE_IN" || type === "SLIDE_OUT" ||
            type === "MOVE_IN" || type === "MOVE_OUT") {
            const { axis, sign } = getSlideAxis(type, dir);
...
function generateAndroid(specs) {
    const lines = [
        "// Generated by figma-intelligence-layer animation-specifier",
...
// ─── Main handler ─────────────────────────────────────────────────────────────
async function animationSpecifierHandler(args) {
    ...
    if (frameNodeId) {
        rawConnections = rawConnections.filter((c) => c.fromId === frameNodeId || c.toId === frameNodeId);
    }
    ...
    const animatedRaw = rawConnections.filter((c) => {
        const transition = c.action["transition"];
