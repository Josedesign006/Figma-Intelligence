...
exports.generateComponentCodeHandler = generateComponentCodeHandler;
const figma_bridge_js_1 = require("../../../shared/figma-bridge.js");
const index_js_1 = require("../../phase5-governance/component-spec/index.js");
...
// ─── Data extraction ────────────────────────────────────────────────────────
async function extractFromFigma(nodeId) {
    const specResult = await (0, index_js_1.componentSpecHandler)({
        nodeId,
        outputFormat: "json",
...
function extractFromBlueprint(blueprint) {
    return {
        name: blueprint.name,
...
// ─── Code generators ────────────────────────────────────────────────────────
function generateReact(data, args) {
    ...
    if (cssModule) {
        ...
        if (hasDisabled)
            classExprParts.push(`disabled ? styles.disabled : ''`);
        if (hasLoading)
            classExprParts.push(`loading ? styles.loading : ''`);
...
function generateVue(data, args) {
    const name = pascalCase(data.name);
    const files = {};
    const stateAxis = data.variantAxes.find((a) => a.name.toLowerCase() === "state");
    const propAxes = data.variantAxes.filter((a) => a.name.toLowerCase() !== "state");
...
function generateSvelte(data, args) {
    const name = pascalCase(data.name);
    const files = {};
    const propAxes = data.variantAxes.filter((a) => a.name.toLowerCase() !== "state");
    const hasLoading = data.states.some((s) => s.toLowerCase() === "loading");
    ...
    if (hasLoading)
        propDecls.push(`  export let loading: boolean = false;`);
    if (hasDisabled)
        propDecls.push(`  export let disabled: boolean = false;`);
    const classExpr = [
        `'${kebabCase(data.name)}'`,
...
function generateHTML(data, args) {
    const name = pascalCase(data.name);
    const files = {};
...
// ─── CSS generation ─────────────────────────────────────────────────────────
function generateCSS(data, args) {
...
function generateCSSBody(data, _args) {
    ...
    if (data.spacing.length > 0) {
        ...
        if (root.itemSpacing > 0)
            lines.push(`  gap: ${root.itemSpacing}px;`);
    ...
    if (data.typography.length > 0) {
        ...
        if (firstText.lineHeightPx)
            lines.push(`  line-height: ${firstText.lineHeightPx}px;`);
    ...
    const typeAxis = propAxes.find((a) => a.name.toLowerCase() === "type" || a.name.toLowerCase() === "variant");
    if (typeAxis) {
        ...
        for (const variant of typeAxis.values) {
            ...
            const variantTokens = data.colorTokens.filter((ct) => ct.tokenName?.toLowerCase().includes(variant.toLowerCase()));
            if (variantTokens.length > 0) {
                ...
                if (bgToken)
                    lines.push(`  background-color: ${bgToken.tokenName ? tokenToCssVar(bgToken.tokenName) : bgToken.colorHex};`);
                if (textToken)
                    lines.push(`  color: ${textToken.tokenName ? tokenToCssVar(textToken.tokenName) : textToken.colorHex};`);
            }
            lines.push(`}`);
            // Hover / active states
...
function generateStories(data, name, ext, propAxes) {
    const typeAxis = propAxes.find((a) => a.name.toLowerCase() === "type" || a.name.toLowerCase() === "variant");
    const sizeAxis = propAxes.find((a) => a.name.toLowerCase() === "size");
    const hasLoading = data.states.some((s) => s.toLowerCase() === "loading");
    const hasDisabled = data.states.some((s) => s.toLowerCase() === "disabled");
    ...
    if (hasDisabled)
        argTypes.push(`    disabled: { control: 'boolean' },`);
    if (hasLoading)
        argTypes.push(`    loading: { control: 'boolean' },`);
    const defaultArgs = [
        `    children: '${name}',`,
...
function generateVueStories(data, name, propAxes) {
    const defaultArgs = propAxes.map((a) => `      ${camelCase(a.name)}: '${a.defaultValue.toLowerCase()}',`);
...
// ─── Main handler ───────────────────────────────────────────────────────────
async function generateComponentCodeHandler(args) {
    let data;
    ...
    if (args.nodeId) {
        ...
        const blueprint = component_templates_js_1.COMPONENT_BLUEPRINTS.find((b) => b.name.toLowerCase() === args.componentName.toLowerCase());
        if (!blueprint) {
            return {
                error: `No blueprint found for component "${args.componentName}".`,
                availableBlueprints: component_templates_js_1.COMPONENT_BLUEPRINTS.map((b) => b.name),
            };
        }
