...
function dtcgType(type, name) {
    if (type === "COLOR")
        return "color";
    if (name.includes("spacing") || name.includes("space") || name.includes("gap") || name.includes("inset") || name.includes("padding"))
        return "dimension";
    if (name.includes("radius"))
        return "dimension";
    if (name.includes("border-width"))
        return "dimension";
    if (name.includes("font-size") || name.includes("text-"))
        return "dimension";
...
async function exportTokensHandler(args) {
    ...
    const formats = args.format === "all"
        ? ["css", "scss", "tailwind", "style-dictionary", "dtcg", "swift", "kotlin", "json"]
        : [args.format];
    const outputs = {};
