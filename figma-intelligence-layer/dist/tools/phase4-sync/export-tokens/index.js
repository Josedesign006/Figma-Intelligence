...
exports.exportTokensHandler = exportTokensHandler;
...
const token_math_js_1 = require("../../../shared/token-math.js");
// ─── Helpers ────────────────────────────────────────────────────────────────
...
function tokenNameToSwiftCase(name) {
    const parts = name.replace(/\//g, "-").replace(/\s+/g, "-").split("-").filter(Boolean);
    return parts
        .map((p, i) => (i === 0 ? p.toLowerCase() : p.charAt(0).toUpperCase() + p.slice(1).toLowerCase()))
        .join("");
}
function tokenNameToKotlinCase(name) {
    const parts = name.replace(/\//g, "-").replace(/\s+/g, "-").split("-").filter(Boolean);
    return parts
        .map((p) => p.charAt(0).toUpperCase() + p.slice(1).toLowerCase())
        .join("");
}
function tokenNameToCamelCase(name) {
    const parts = name
        .replace(/\//g, "-")
        .replace(/\s+/g, "-")
        .split("-")
...
function tokenNameToSnakeCase(name) {
    return name
        .replace(/\//g, "_")
        .replace(/\s+/g, "_")
...
function tokenNameToDtcgPath(name) {
...
function figmaColorToHex(color) {
    ...
    const b = color.b ?? 0;
    const a = color.a ?? 1;
    const hex = (0, token_utils_js_1.figmaRgbaToHex)(r, g, b);
    if (a < 1) {
...
function resolveValue(raw, type) {
...
function parseShadowString(val) {
...
function parseCubicBezierString(val) {
...
function parseTypographyString(val) {
...
function parseTransitionString(val) {
...
// ─── Catalog fallback ───────────────────────────────────────────────────────
function catalogToTokens(args) {
    return semantic_token_catalog_js_1.SEMANTIC_TOKEN_CATALOG
        .filter((entry) => {
        if (args.tokenTypes?.length && !args.tokenTypes.includes(entry.type))
            return false;
        if (args.collectionFilter && !entry.category.toLowerCase().includes(args.collectionFilter.toLowerCase()))
            return false;
        return true;
    })
        .map((entry) => {
...
function parseCatalogRef(ref, type) {
    if (type === "FLOAT") {
...
// ─── Format generators ──────────────────────────────────────────────────────
function generateCSS(tokens, args) {
...
function generateSCSS(tokens, args) {
    const modes = collectModes(tokens);
    const lines = [
...
function generateTailwind(tokens, args) {
...
function generateStyleDictionary(tokens, args) {
    const modes = collectModes(tokens);
    const result = {};
    for (const token of tokens) {
        ...
        if (token.aliasOf)
            leaf.original = { value: `{${token.aliasOf.replace(/\//g, ".")}}` };
        // Modes as attributes
        if (modes.length > 1) {
            const modeVals = {};
...
function dtcgType(type, name) {
    if (type === "COLOR")
        return "color";
    if (type === "FLOAT") {
        if (name.includes("opacity") || name.includes("z-index"))
            return "number";
    ...
    if (type === "STRING") {
        if (name.includes("easing") || name.includes("timing") || name.includes("cubic-bezier"))
            return "cubicBezier";
        ...
        if (name.includes("typography"))
            return "typography";
        if (name.includes("border") && !name.includes("border-width") && !name.includes("border-radius"))
            return "border";
...
function dtcgValue(val, type, dtcgTypeName) {
    switch (dtcgTypeName) {
        case "color":
...
function generateDTCG(tokens, args) {
...
function generateSwift(tokens, args) {
    const modes = collectModes(tokens);
    ...
    if (colorTokens.length > 0) {
        ...
        if (modes.includes("Dark")) {
            ...
            for (const token of colorTokens) {
                ...
                if (darkVal === undefined || typeof darkVal !== "string")
                    continue;
...
function generateKotlin(tokens, args) {
    const colorTokens = tokens.filter((t) => t.type === "COLOR");
    const floatTokens = tokens.filter((t) => t.type === "FLOAT");
    const modes = collectModes(tokens);
    const lines = [
...
function generateJSON(tokens, _args) {
...
function generateFlutter(tokens, _args) {
    const colorTokens = tokens.filter((t) => t.type === "COLOR");
    const floatTokens = tokens.filter((t) => t.type === "FLOAT");
    const stringTokens = tokens.filter((t) => t.type === "STRING");
    const lines = [
...
function generateAndroidXml(tokens, _args) {
    const lines = [
...
function generateJsModule(tokens, _args) {
    const lines = [
...
function generateTsModule(tokens, _args) {
    const lines = [
...
function generateLess(tokens, _args) {
    const modes = collectModes(tokens);
    const lines = [
...
function generateReactNative(tokens, _args) {
    const colorTokens = tokens.filter((t) => t.type === "COLOR");
    const floatTokens = tokens.filter((t) => t.type === "FLOAT");
    const lines = [
    ...
    const spacingTokens = floatTokens.filter((t) => t.name.includes("space") || t.name.includes("spacing") || t.name.includes("gap") || t.name.includes("padding") || t.name.includes("inset"));
    ...
    const otherFloats = floatTokens.filter((t) => !spacingTokens.includes(t) && !radiusTokens.includes(t) && !fontSizeTokens.includes(t));
...
function generateTailwindV4(tokens, args) {
    const prefix = args.tailwindPrefix ?? "ds";
    const lines = [
...
function generateCssRem(tokens, args) {
    const selector = args.cssSelector ?? ":root";
    ...
    for (const mode of modes) {
        ...
        for (const token of tokens) {
            ...
            if (token.type === "COLOR" && typeof val === "string") {
                ...
                // Convert px to rem
                cssVal = (0, token_math_js_1.pxToRem)(val, remBase);
            }
            else {
...
function formatCssValue(val, type) {
...
function formatRawValue(val, type) {
    if (type === "COLOR" && typeof val === "string")
        return val;
    return val;
}
function sdType(type) {
    switch (type) {
...
// ─── Main handler ───────────────────────────────────────────────────────────
async function exportTokensHandler(args) {
