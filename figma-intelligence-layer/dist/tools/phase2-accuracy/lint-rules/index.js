...
exports.BUILT_IN_RULES = void 0;
exports.lintRulesHandler = lintRulesHandler;
...
async function getAllRules() {
    ...
    const overriddenBuiltIn = exports.BUILT_IN_RULES.map((br) => {
...
function isButton(node) {
    ...
    return (/\bbtn\b|\bbutton\b|\bcta\b/.test(name) ||
        node.type === "INSTANCE" ||
        node.type === "COMPONENT");
}
...
function hasDetachedTextColor(node) {
    if (node.type !== "TEXT")
        return false;
    return hasHardcodedColor(node.fills);
}
// ─── Auto-fix builders ────────────────────────────────────────────────────────
function buildSnapSpacingScript(nodeId, snappedPadding, snappedGap) {
    return `
    (async () => {
...
function buildMapColorToTokenScript(nodeId, tokenName) {
    return `
    (async () => {
...
// ─── Violation checker ────────────────────────────────────────────────────────
async function runChecks(nodes, rules, autoFix, dsTokens, bridge) {
    const violations = [];
    ...
    for (const node of nodes) {
        ...
        if (activeRuleIds.has("no-hardcoded-colors") && hasHardcodedColor(node.fills)) {
            ...
            if (autoFix && rule.autoFixable) {
                ...
                if (hardcoded?.color) {
                    ...
                    const tokenRef = (0, token_utils_js_1.snapToColorToken)(hex, dsTokens);
        ...
        if (activeRuleIds.has("spacing-on-grid")) {
            ...
            if (offGridFields.length > 0) {
                let fixed = false;
                if (autoFix && rule.autoFixable) {
                    const paddingRef = (0, token_utils_js_1.snapToSpacingToken)(node.paddingLeft ?? 0);
                    const gapRef = (0, token_utils_js_1.snapToSpacingToken)(node.itemSpacing ?? 0);
                    const fixScript = buildSnapSpacingScript(node.id, paddingRef.tokenValue, gapRef.tokenValue);
                    const fixResult = await bridge.execute(fixScript);
                    if (fixResult.success) {
                        fixed = true;
                        autoFixedCount++;
...
// ─── Output formatters ────────────────────────────────────────────────────────
function formatOutput(violations, format, totalChecked, autoFixedCount) {
    if (format === "ci") {
...
async function actionDefine(args) {
    if (!args.ruleFile) {
...
async function actionDelete(args) {
    if (!args.ruleFile) {
...
async function actionRun(args) {
    ...
    `);
        ...
        rootNode = execResult.result;
    }
    const nodes = collectAllNodes(rootNode);
    const { violations, autoFixedCount } = await runChecks(nodes, rules, args.autoFix ?? false, dsTokens, bridge);
...
// ─── Main handler ─────────────────────────────────────────────────────────────
async function lintRulesHandler(args) {
    switch (args.action) {
        case "define":
        ...
        default:
            throw new Error(`lintRules: Unknown action "${args.action}". Valid actions: define | run | list | delete.`);
    }
}
